...
#		pragma clang diagnostic ignored \
		    "-Wfloat-equal"        // warning: comparing floating point with ==
		                           ...
		                           // against same constants (typically 0.0f) is
		                           // ok.
#		pragma clang diagnostic ignored \
		    "-Wformat"        // warning: format specifies type 'int' but the
		                      // argument has type 'unsigned int'
#		pragma clang diagnostic ignored \
		    "-Wformat-nonliteral"        // warning: format string is not a
		                                 // string literal            // passing
		                                 // non-literal to vsnformat(). yes,
		                                 // user passing incorrect format
...
#		pragma clang diagnostic ignored \
		    "-Wunused-macros"        // warning: macro is not used // we define
		                             // snprintf/vsnprintf on Windows so they
...
#		pragma clang diagnostic ignored \
		    "-Wdouble-promotion"        // warning: implicit conversion from
		                                ...
		                                // double.
#		pragma clang diagnostic ignored \
		    "-Wenum-enum-conversion"        // warning: bitwise operation
...
#		pragma GCC diagnostic ignored \
		    "-Wfloat-equal"        // warning: comparing floating-point with
		                           // '==' or '!=' is unsafe
#		pragma GCC diagnostic ignored \
		    "-Wformat"        // warning: format '%p' expects argument of type
		                      // 'int'/'void*', but argument X has type
		                      // 'unsigned int'/'ImGuiWindow*'
#		pragma GCC diagnostic ignored \
		    "-Wformat-nonliteral"        // warning: format not a string
		                                 // literal, format string not checked
#		pragma GCC diagnostic ignored \
		    "-Wdeprecated-enum-enum-conversion"        // warning: bitwise
		                                               // operation between
		                                               // different enumeration
...
static const float DRAG_MOUSE_THRESHOLD_FACTOR =
    0.50f;        // Multiplier for the default value of io.MouseDragThreshold
                  // to make DragFloat/DragInt react faster to mouse drags.
...
static const ImS32          IM_S32_MIN =
    INT_MIN;        // (-2147483647 - 1), (0x80000000);
...
// Note that all functions taking format strings in the API may be passed ("%s",
// text) or ("%.*s", text_len, text), which will automatically bypass the
// formatter.
...
// item on the line. (#7024, #9455)
void ImGui::TextAlignedV(float       align_x,
                         float       width,
                         const char *fmt,
                         va_list     args)
{
	...
	window->DC.CursorMaxPos.x =
	    backup_max_pos.x;        // Cancel out extending content to 'size' to
	                             // use 'text_size' allowing auto-resize to
	                             // function with align_x > 0.0f
	if (!is_visible)
		return;
	...
	RenderTextEllipsis(
	    window->DrawList, pos, pos_max, pos_max.x, text, text_end, &text_size);
...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args)
{
	...
	const ImRect total_bb(
	    pos,
	    pos + ImVec2(w + (label_size.x > 0.0f ?
...
#	endif
		...
		for (int button = 0; button < 3; button++)
			if (flags &
			    (ImGuiButtonFlags_MouseButtonLeft
			     << button))        // Handle ImGuiButtonFlags_MouseButtonRight
			                        // and ImGuiButtonFlags_MouseButtonMiddle
			                        // here.
			{
				if (IsMouseClicked(
				        button, ImGuiInputFlags_None, test_owner_id) &&
		...
		if (mods_ok) {
			if (mouse_button_clicked != -1 && g.ActiveId != id) {
				...
				if (flags & ImGuiButtonFlags_PressedOnRelease) {
					// FIXME: Traditionally ImGuiButtonFlags_PressedOnRelease
					// never took ActiveId. Adding it in 2026-03-20 since
					// ImGuiButtonFlags_NoHoldingActiveId can always be added.
					// We don't yet perform an explicit ClearActiveID() to
					// reduce scope of change, but this possibility could be
					// investigated.
					if (!(flags & ImGuiButtonFlags_NoHoldingActiveId))
						SetActiveID(id, window);        // Hold on ID
					g.ActiveIdMouseButton = (ImS8) mouse_button_clicked;
			...
			if (flags & ImGuiButtonFlags_PressedOnRelease) {
				if (mouse_button_released != -1) {
					...
					if (!(flags & ImGuiButtonFlags_NoNavFocus))
						SetFocusID(
						    id,
						    window);        // FIXME: Lack of FocusWindow() call
						                    // here is inconsistent with other
						                    // paths. Research why.
					ClearActiveID();
				}
			}
	...
	if (g.ActiveId == id) {
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			if (g.NavActivateDownId == id)
				held =
				    true;        // hovered == true not true as we are already
                     ...
                     ImGuiButtonFlags flags)
{
	...
	if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
	    style.FramePadding.y <
	        window->DC
	            .CurrLineTextBaseOffset)        // Try to vertically align
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos)
{
	...
	const float  cross_thickness =
	    1.0f * (float) (int) g.Style._MainScale;        // FIXME-DPI
...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow *window, ImGuiAxis axis)
{
	...
	const float   scrollbar_size =
	    window
	        ->ScrollbarSizes[axis ^ 1];        // (ScrollbarSizes.x = width of Y
...
void ImGui::ExtendHitBoxWhenNearViewportEdge(ImGuiWindow *window,
                                             ImRect      *bb,
                                             float        threshold,
                                             ImGuiAxis    axis)
{
	...
	if (window_rect.Min[axis] == viewport_rect.Min[axis] &&
	    bb->Min[axis] > window_rect.Min[axis] &&
	    bb->Min[axis] - threshold <= window_rect.Min[axis])
		bb->Min[axis] = window_rect.Min[axis];
	if (window_rect.Max[axis] == viewport_rect.Max[axis] &&
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
	...
	ImRect host_rect =
	    (window->DockIsActive ? window->DockNode->HostWindow : window)->Rect();
	ImDrawFlags rounding_corners =
	    CalcRoundingFlagsForRectInRect(bb, host_rect, g.Style.WindowBorderSize);
	float size_visible =
                        ...
                        ImDrawFlags   draw_rounding_flags)
{
	...
	const float grab_h_pixels  = (float) (int) ImClamp(
	    scrollbar_size_v * ((float) size_visible_v / (float) win_size_v),
	    grab_h_minsize,
	    scrollbar_size_v);
	const float grab_h_norm = grab_h_pixels / scrollbar_size_v;

	// As a special thing, we allow scrollbar near the edge of a screen/viewport
	// to be reachable with mouse at the extreme edge (#9276)
	ImRect bb_hit = bb_frame;
	ExtendHitBoxWhenNearViewportEdge(
	    window, &bb_hit, g.Style.WindowBorderSize, (ImGuiAxis) (axis ^ 1));
	...
	if (axis == ImGuiAxis_X)
		grab_rect =
		    ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm),
		           bb.Min.y,
...
// above.
void ImGui::ImageWithBg(ImTextureRef  tex_ref,
                        ...
                        const ImVec4 &tint_col)
{
	...
	if (bg_col.w > 0.0f)
		window->DrawList->AddRectFilled(
		    bb.Min + padding, bb.Max - padding, GetColorU32(bg_col), rounding);
	if (rounding > 0.0f)
		window->DrawList->AddImageRounded(tex_ref,
		                                  bb.Min + padding,
		                                  bb.Max - padding,
		                                  uv0,
		                                  uv1,
		...
		window->DrawList->AddImage(tex_ref,
		                           bb.Min + padding,
		                           bb.Max - padding,
		                           uv0,
		                           uv1,
...
void ImGui::Image(ImTextureRef  tex_ref,
                  const ImVec2 &image_size,
                  const ImVec2 &uv0,
                  const ImVec2 &uv1)
{
                        ...
                        const ImVec4 &tint_col)
{
	...
	return ImageButtonEx(
	    window->GetID(str_id), tex_ref, image_size, uv0, uv1, bg_col, tint_col);
}
...
bool ImGui::Checkbox(const char *label, bool *v)
{
	...
	if (!is_visible)
		if (!is_multi_select || !g.BoxSelectState.UnclipMode ||
		    !g.BoxSelectState.UnclipRect.Overlaps(
		        total_bb))        // Extra layer of "no logic clip" for
		                          // box-select support
		{
	...
	if (is_multi_select)
		MultiSelectItemFooter(
		    id, &checked, &pressed, ImGuiMultiSelectFlags_CheckboxMode_);
	else if (pressed)
		checked = !checked;
	...
	if (is_visible) {
		...
		RenderFrame(
		    check_bb.Min, check_bb.Max, bg_col, true, style.FrameRounding);
		if (mixed_value) {
...
bool ImGui::RadioButton(const char *label, bool active)
{
	...
	if (active) {
		const float pad = ImMax(1.0f, IM_TRUNC(square_sz / 6.0f));
		window->DrawList->AddCircleFilled(
		    center, radius - pad, GetColorU32(ImGuiCol_CheckMark));
	}
                        ...
                        const char   *overlay)
{
	...
	if (fill_x0 < fill_x1)
		RenderRectFilledInRangeH(window->DrawList,
		                         bb,
	...
	if (!is_indeterminate || overlay != NULL) {
		...
		if (overlay_size.x > 0.0f) {
			...
			RenderTextClipped(ImVec2(ImClamp(text_x,
			                                 bb.Min.x,
...
bool ImGui::TextLink(const char *label)
{
	...
	window->DrawList->AddLineH(
	    ...
	    1.0f *
	        (float) (int) g.Style
	            ._MainScale);        // FIXME-TEXT: Underline mode // FIXME-DPI
...
bool ImGui::TextLinkOpenURL(const char *label, const char *url)
{
	...
	SetItemTooltip(LocalizeGetMsg(ImGuiLocKey_OpenLink_s),
	               url);        // It is more reassuring for user to _always_
	                            // display URL when it is the same as the label.
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
	...
	if (flags & ImGuiSeparatorFlags_Vertical) {
		...
		if (columns) {
			x1 = window->Pos.x +
			     window->DC.Indent
			         .x;        // Used to be Pos.x before 2023/10/03
		...
		// Between 1.71 and 1.92.7, we maintained a hack where a 1.0f thin
		// Separator() would not impact layout. This was mostly chosen to allow
		// backward compatibility with user's code assuming zero-height when
		// calculating height for layout (e.g. bottom alignment of a status
		// bar). In order to handle scaling we need to scale separator thickness
                            ...
                            float       extra_w)
{
	...
	const float  text_baseline_y = ImTrunc(
	    ...
	    0.999f);        // ImMax(padding.y, ImTrunc((style.SeparatorTextSize -
	                    // label_size.y) * 0.5f));
	...
	if (label_size.x > 0.0f) {
		...
		if (sep1_x2 > sep1_x1 && separator_thickness > 0.0f)
			window->DrawList->AddLineH(
			    sep1_x1, sep1_x2, seps_y, separator_col, separator_thickness);
		if (sep2_x2 > sep2_x1 && separator_thickness > 0.0f)
			window->DrawList->AddLineH(
			    sep2_x1, sep2_x2, seps_y, separator_col, separator_thickness);
		if (g.LogEnabled)
		...
		if (separator_thickness > 0.0f)
			window->DrawList->AddLineH(
			    sep1_x1, sep2_x2, seps_y, separator_col, separator_thickness);
	}
}
...
#	endif
	...
	if (hovered)
		g.LastItemData.StatusFlags |=
		    ImGuiItemStatusFlags_HoveredRect;        // for IsItemHovered(),
		                                             // because bb_interact is
		                                             // larger than bb
	...
	if (bg_col & IM_COL32_A_MASK)
		window->DrawList->AddRectFilled(
		    bb_render.Min, bb_render.Max, bg_col, 0.0f);
	const ImU32 col =
                            ...
                            ImGuiComboFlags flags)
{
	...
	if (ImGuiWindow *popup_window = FindWindowByName(name))
		if (popup_window->WasActive) {
			...
			ImVec2 pos =
			    FindBestWindowPosForPopupEx(bb.GetBL(),
			                                size_expected,
			                                &popup_window->AutoPosLastDirection,
			                                r_outer,
			                                bb,
	...
	if (!ret) {
		...
		if (!g.IO.ConfigDebugBeginReturnValueOnce &&
		    !g.IO.ConfigDebugBeginReturnValueLoop)        // Begin may only
		                                                  // return false with
		                                                  // those debug tools
		                                                  // activated.
...
bool ImGui::BeginComboPreview()
{
	...
	IM_ASSERT(g.LastItemData.Rect.Min.x == preview_data->PreviewRect.Min.x &&
	          g.LastItemData.Rect.Min.y ==
	              preview_data->PreviewRect.Min
	                  .y);        // Didn't call after BeginCombo/EndCombo block
	                              // or forgot to pass
	...
	window->DC.IsSameLine   = false;
	PushClipRect(
	    preview_data->PreviewRect.Min, preview_data->PreviewRect.Max, true);
                  ...
                  int         height_in_items)
{
	int         items_count = 0;
	const char *p =
	    items_separated_by_zeros;        // FIXME-OPT: Avoid computing this, or
	                                     // at least only when combo is open
	while (*p) {
		p += ImStrlen(p) + 1;
		items_count++;
	}
...
static const ImU32 GDefaultRgbaColorMarkers[4] = {IM_COL32(240, 20, 20, 255),
...
#	endif
    {sizeof(float),
     "float",
     "%.3f",
     "%f"},        // ImGuiDataType_Float (float are promoted to double in
                   // va_arg)
                                  ...
                                  void         *p_data_when_empty)
{
	...
	if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double) {
		...
		format = ImParseFormatSanitizeForScanning(
		    format, format_sanitized, IM_COUNTOF(format_sanitized));
		if (format[0] == '\0')
			format =
			    type_info->ScanFmt;        // Format doesn't want us to show the
			                               // number currently, but we still
			                               // need to parse the resulting input
	}
                                   ...
                                   TYPE          v)
{
	...
	if (fmt_start[0] != '%' ||
	    fmt_start[1] == '%')        // Don't apply if the value is not visible
	...
	ImParseFormatSanitizeForPrinting(
	    fmt_start, fmt_sanitized, IM_COUNTOF(fmt_sanitized));
...
static bool TempInputIsClampEnabled(ImGuiSliderFlags flags,
                                    ImGuiDataType    data_type,
                                    const void      *p_min,
                                    const void      *p_max)
{
	if ((flags & ImGuiSliderFlags_ClampOnInput) &&
		...
		if (clamp_range_dir == 0)
			return ImGui::DataTypeIsZero(data_type, p_min) ?
                       ...
                       ImGuiSliderFlags flags)
{
	...
	if (temp_input_is_active) {
		const bool clamp_enabled =
		    TempInputIsClampEnabled(flags, data_type, p_min, p_max);
	...
	RenderFrame(
	    frame_bb.Min, frame_bb.Max, frame_col, false, style.FrameRounding);
	if (color_marker != 0 && style.ColorMarkerSize > 0.0f)
		RenderColorComponentMarker(
		    frame_bb, GetColorU32(color_marker), style.FrameRounding);
	...
	const char *value_buf_end =
	    value_buf +
	    DataTypeFormatString(
	        value_buf, IM_COUNTOF(value_buf), data_type, p_data, format);
	if (g.LogEnabled)
		LogSetNextTextDecoration("{", "}");
...
{
	return DragScalar(
	    label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format, flags);
}
...
{
	return DragScalar(
	    label, ImGuiDataType_S32, v, v_speed, &v_min, &v_max, format, flags);
}
...
{
	return DragScalarN(
	    label, ImGuiDataType_S32, v, 2, v_speed, &v_min, &v_max, format, flags);
}
...
{
	return DragScalarN(
	    label, ImGuiDataType_S32, v, 3, v_speed, &v_min, &v_max, format, flags);
}
...
{
	return DragScalarN(
	    label, ImGuiDataType_S32, v, 4, v_speed, &v_min, &v_max, format, flags);
}
                                  ...
                                  float         zero_deadzone_halfsize)
{
	...
	if (logarithmic_zero_epsilon > 0.0f)        // == is_logarithmic from caller
	{
		...
		if (v_clamped <= v_min_fudged)
			result = 0.0f;        // Workaround for values that are in-range but
			                      // below our fudge
		else if (v_clamped >= v_max_fudged)
			result = 1.0f;        // Workaround for values that are in-range but
			                      // above our fudge
		else if ((v_min * v_max) <
		         0.0f)        // Range crosses zero, so split into two portions
		{
			    ...
			    ((float) v_max -
			     (float)
			         v_min);        // The zero point in parametric space.
			                        ...
			                        // calculating this, but for now this should
			                        // do (and the most common case of a
			...
			if (v == 0.0f)
				result =
				    zero_point_center;        // Special case for exactly zero
			else if (v < 0.0f)
				result = (1.0f - (float) (ImLog(-(FLOATTYPE) v_clamped /
				                                logarithmic_zero_epsilon) /
				                          ImLog(-v_min_fudged /
				                                logarithmic_zero_epsilon))) *
				         zero_point_snap_L;
                                 ...
                                 float         zero_deadzone_halfsize)
{
	...
	if (logarithmic_zero_epsilon > 0.0f)        // == is_logarithmic from caller
	{
		...
		if ((v_min * v_max) < 0.0f)        // Range crosses zero, so we have to
		                                   // do this in two parts
		{
			...
			if (t_with_flip >= zero_point_snap_L &&
				...
				result = (TYPE) 0.0f;        // Special case to make getting
				                             // exactly zero possible (the
	...
	if (g.ActiveId == id) {
		...
		if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
			...
			if (input_delta != 0.0f) {
				...
				if (decimal_precision > 0) {
					input_delta /=
					    100.0f;        // Keyboard/Gamepad tweak speeds in % of
					                   // slider bounds
					if (tweak_slow)
						input_delta /= 10.0f;
				} else {
                           ...
                           ImRect          *out_grab_bb)
{
	...
	switch (data_type) {
		...
		case ImGuiDataType_S16:
		{
			ImS32 v32 = (ImS32) * (ImS16 *) p_v;
			bool  r =
			    SliderBehaviorT<ImS32, ImS32, float>(bb,
			                                         id,
			                                         ImGuiDataType_S32,
			                                         &v32,
			                                         *(const ImS16 *) p_min,
		...
		case ImGuiDataType_U16:
		{
			ImU32 v32 = (ImU32) * (ImU16 *) p_v;
			bool  r =
			    SliderBehaviorT<ImU32, ImS32, float>(bb,
			                                         id,
			                                         ImGuiDataType_U32,
			                                         &v32,
			                                         *(const ImU16 *) p_min,
                         ...
                         ImGuiSliderFlags flags)
{
	...
	if (temp_input_is_active) {
		...
		const bool clamp_enabled =
		    ...
		    0;        // Don't use TempInputIsClampEnabled()
		return TempInputScalar(frame_bb,
		                       id,
		                       label,
		                       data_type,
...
{
	return SliderScalar(
	    label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}
                         ...
                         ImGuiSliderFlags flags)
{
	return SliderScalarN(
	    label, ImGuiDataType_Float, v, 2, &v_min, &v_max, format, flags);
}
...
{
	return SliderScalarN(
	    label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format, flags);
}
...
{
	return SliderScalarN(
	    label, ImGuiDataType_Float, v, 4, &v_min, &v_max, format, flags);
}
...
{
	return SliderScalar(
	    label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
{
	return SliderScalarN(
	    label, ImGuiDataType_S32, v, 2, &v_min, &v_max, format, flags);
}
...
{
	return SliderScalarN(
	    label, ImGuiDataType_S32, v, 3, &v_min, &v_max, format, flags);
}
...
{
	return SliderScalarN(
	    label, ImGuiDataType_S32, v, 4, &v_min, &v_max, format, flags);
}
                         ...
                         ImGuiSliderFlags flags)
{
	return VSliderScalar(
	    label, size, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}
...
{
	return VSliderScalar(
	    label, size, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
const char *
    ImParseFormatTrimDecorations(const char *fmt, char *buf, size_t buf_size)
{
	...
	if (fmt_end[0] == 0)        // If we only have leading decoration, we don't
		...
		return fmt_start;
	buf[0] = 0;        // Fix false positive with caller assuming that buf could
	                   // be non-initialized (#9508)
	ImStrncpy(
                                      ...
                                      size_t      fmt_out_size)
{
	...
	while (fmt_in < fmt_end) {
		char c = *fmt_in++;
		if (c != '\'' && c != '$' &&
		    c != '_')        // Custom flags provided by stb_sprintf.h. POSIX
		                     // 2008 also supports '.
			*(fmt_out++) = c;
	}
                          ...
                          void                  *user_data)
{
	...
	g.LastItemData.ItemFlags |=
	    ImGuiItemFlags_AllowDuplicateId;        // Using
	                                            // ImGuiInputTextFlags_MergedItem
	                                            // above will skip ItemAdd() so
	                                            // we poke here.
	bool value_changed = InputTextEx(label,
	                                 ...
	                                 (int) buf_size,
	                                 bb.GetSize(),
	                                 flags | ImGuiInputTextFlags_TempInput |
                            ...
                            const void   *p_clamp_max)
{
	...
	DataTypeFormatString(
	    data_buf, IM_COUNTOF(data_buf), data_type, p_data, format);
	...
	g.LastItemData.ItemFlags |=
	    ImGuiItemFlags_NoMarkEdited;        // Because TempInputText() uses
	                                        ...
	                                        // doesn't submit a new item, so we
	                                        // poke LastItemData.
	g.LastItemData.ItemFlags =
	    (g.LastItemData.ItemFlags & ImGuiItemFlags_LiveEditOnInputScalar) ?
	        (g.LastItemData.ItemFlags | ImGuiItemFlags_LiveEditOnInputText) :
	        (g.LastItemData.ItemFlags & ~ImGuiItemFlags_LiveEditOnInputText);
                        ...
                        ImGuiInputTextFlags flags)
{
	...
	ImGuiContext &g     = *GImGui;
	ImGuiStyle   &style = g.Style;
	// IM_ASSERT((flags & ImGuiInputTextFlags_EnterReturnsTrue) == 0); // Not
	// supported by InputScalar(). Please open an issue if you this would be
	// useful to you. Otherwise use IsItemDeactivatedAfterEdit()!
	...
	if (g.LastItemData.ItemFlags & ImGuiItemFlags_LiveEditOnInputScalar) {
		bool input_edited =
		    ...
		    0;        // We would be using 'ret' if
		              // ImGuiInputTextFlags_EnterReturnsTrue was not involved.
		if (input_edited)
		...
		if (g.DeactivatedItemData.ID == g.LastItemData.ID) {
			// g.DeactivatedItemData.HasBeenEditedBefore = false; // Will be set
			// below by MarkItemEdited() if (IsItemDeactivated()) // Should be
			// unnecessary
			value_changed = DataTypeApplyFromText(
			    buf,
			    data_type,
                                    ...
                                    ImDrawTextFlags flags)
{
	...
	return ImFontCalcTextSizeEx(g.Font,
	                            g.FontSize,
	                            FLT_MAX,
	                            obj->WrapWidth,
	                            text_begin,
                                                ...
                                                int                  idx)
{
	...
	unsigned int c;
	return idx + ImTextCharFromUtf8(
	                 &c, obj->TextSrc + idx, obj->TextSrc + obj->TextLen);
}
                                           ...
                                           int                       cursor)
{
	...
	if (obj->WrapWidth > 0.0f) {
		...
		while (p >= p_bol) {
			const char *p_eol =
			    ImFontCalcWordWrapPositionEx(g.Font,
			                                 g.FontSize,
			                                 p,
			                                 text_end,
			                                 obj->WrapWidth,
			                                 ImDrawTextFlags_WrapKeepBlanks);
			if (p == p_cursor)        // If we are already on a visible
			                          // beginning-of-line, return real
			                          // beginning-of-line (would be same as
...
static int STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState *obj,
                                    int                  pos,
                                    const char          *new_text,
                                    int                  new_text_len)
{
		...
		new_text_len =
		    (int) (ImTextFindValidUtf8CodepointEnd(
		           ...
		           new_text);        // Truncate to closest UTF-8 codepoint.
		                             // Alternative: return 0 to cancel
		                             // insertion.
	if (new_text_len == 0)
		return 0;
	...
	if (pos != text_len)
		memmove(
		    text + pos + new_text_len, text + pos, (size_t) (text_len - pos));
...
#	define STB_TEXTEDIT_K_LEFT \
		0x200000        // keyboard input to move cursor left
#	define STB_TEXTEDIT_K_RIGHT \
		0x200001        // keyboard input to move cursor right
#	define STB_TEXTEDIT_K_UP \
		0x200002        // keyboard input to move cursor up
#	define STB_TEXTEDIT_K_DOWN \
		0x200003        // keyboard input to move cursor down
...
#	define STB_TEXTEDIT_K_DELETE \
		0x200008        // keyboard input to delete selection or character under
		                // cursor
#	define STB_TEXTEDIT_K_BACKSPACE \
		0x200009        // keyboard input to delete selection or character left
		                // of cursor
#	define STB_TEXTEDIT_K_UNDO \
		0x20000A        // keyboard input to perform undo
#	define STB_TEXTEDIT_K_REDO \
		0x20000B        // keyboard input to perform redo
...
#	define STB_TEXTEDIT_K_PGUP \
		0x20000E        // keyboard input to move cursor up a page
                                 ...
                                 int                            text_len)
{
	...
	IM_ASSERT(
	    0);        // Failed to insert character, normally shouldn't happen
                                             ...
                                             const char *new_text_end)
{
	...
	int        new_text_len = new_text_end ? (int) (new_text_end - new_text) :
	                                         (int) ImStrlen(new_text);
                                     ...
                                     bool input_source_is_clipboard)
{
	...
	if (c < 0x20) {
		...
		if (!pass)
			return false;
		apply_named_filters =
		    false;        // Override named filters below so newline and tabs
...
void ImGui::InputTextDeactivateHook(ImGuiID id)
{
	...
	if (state->Flags & ImGuiInputTextFlags_ReadOnly) {
		g.InputTextDeactivatedState.TextA.resize(
		    0);        // In theory this data won't be used, but clear to be
		               // neat.
	} else {
                                   ...
                                   const char        **out_buf_end)
{
	...
	if (flags & ImGuiInputTextFlags_WordWrap) {
		for (s = buf; s < buf_end; s = (*s == '\n') ? s + 1 : s) {
			...
			s = ImFontCalcWordWrapPositionEx(g.Font,
			                                 g.FontSize,
			                                 s,
			                                 buf_end,
			                                 wrap_width,
...
static ImVec2 InputTextLineIndexGetPosOffset(ImGuiContext        &g,
                                             ImGuiInputTextState *state,
                                             ImGuiTextIndex      *line_index,
                                             const char          *buf,
                                             const char          *buf_end,
                                             int                  cursor_n)
{
	...
	ImVec2      offset;
	offset.x = InputTextCalcTextSize(&g,
	                                 line_start,
	                                 cursor_ptr,
	                                 buf_end,
	                                 NULL,
                        ...
                        void                  *callback_user_data)
{
	...
	IM_ASSERT(
	    !((flags & ImGuiInputTextFlags_ElideLeft) &&
	      (flags &
	       ImGuiInputTextFlags_Multiline)));        // Multiline does not not
	...
	if (is_multiline)        // Open group before calling GetID() because groups
	                         // tracks id created within their scope (including
	                         // the scrollbar)
		BeginGroup();
	const ImGuiID id         = window->GetID(label);
	const char   *label_end  = FindRenderedTextEnd(label);
	...
	const ImVec2  frame_size = CalcItemSize(
	    ...
	    (is_multiline ? g.FontSize * 8.0f : label_size.y) +
	        style.FramePadding.y * 2.0f);        // Arbitrary default of 8 lines
	                                             // high for multi-line
	const ImVec2 total_size =
	...
	if (is_multiline) {
		...
		bool no_clip =
		    ...
		    (id ==
		     g.NavActivateId);        // Mimic some of ItemAdd() logic + add
		                              // InputTextDeactivatedState.ID check.
		...
		// FIXME: Could this be a ImGuiChildFlags to affect the
		// SetLastItemDataForWindow() call?
	...
	const bool input_requested_by_reactivate =
	    (g.InputTextReactivateId ==
	     id);        // for io.ConfigInputTextEnterKeepActive
	const bool input_requested_by_user =
	    (user_clicked) ||
	    (g.ActiveId == 0 && (flags & ImGuiInputTextFlags_TempInput) &&
	...
	const ImGuiID scrollbar_id =
	    (is_multiline && state != NULL) ?
	        ...
	        0;
	const bool user_scroll_finish = is_multiline && state != NULL &&
	                                g.ActiveId == 0 &&
	                                g.ActiveIdPreviousFrame == scrollbar_id;
	const bool user_scroll_active =
	    is_multiline && state != NULL && g.ActiveId == scrollbar_id;
	...
	const bool init_changed_specs_readonly =
	    (state != NULL &&
	     ((state->Flags ^ flags) &
	      ImGuiInputTextFlags_ReadOnly));        // state != NULL means its our
	                                             // state.
	const bool init_make_active =
	    (input_requested_by_user || input_requested_by_nav ||
	     input_requested_by_reactivate || user_scroll_finish);
	if (init_reload_from_user_buf) {
		...
		InputTextReconcileUndoState(
		    state, state->TextA.Data, state->TextLen, buf, new_len);
		state->TextA.resize(
	...
	} else if ((init_make_active && g.ActiveId != id) ||
	           init_changed_specs_multiline || init_changed_specs_readonly) {
		...
		if (state->ID != id && state->ID == g.ActiveId &&
		    (init_make_active && g.ActiveId != id))        //-V560
			InputTextDeactivateHook(
			    state->ID);        // <-- this is essentially an earlier call to
			                       // what SetActiveID() would do below.
	...
	if (g.ActiveId == id) {
		...
		const ImGuiKey always_owned_keys[] = {ImGuiKey_LeftArrow,
		                                      ImGuiKey_RightArrow,
		                                      ImGuiKey_Delete,
		                                      ImGuiKey_Backspace,
		                                      ImGuiKey_Home,
	...
	if (g.ActiveId == id && io.MouseClicked[0] &&
	...
	if (state != NULL && state->ID == id) {
		...
		// Word-wrapping: attempt to keep cursor in view while resizing
		// frame/parent (FIXME-WORDWRAP: would be better to preserve same
		// relative offset)
		...
		if (select_all) {
			...
			if ((multiclick_count % 2) == 0) {
				...
				const bool is_eol = ImStb::STB_TEXTEDIT_GETCHAR(
				                        state, state->Stb->cursor) == '\n';
				state->WrapWidth  = 0.0f;        // Temporarily disable wrapping
				                                 // so we use real line start.
		...
		if ((flags & ImGuiInputTextFlags_AllowTabInput) && !is_readonly) {
			if (Shortcut(ImGuiKey_Tab, ImGuiInputFlags_Repeat, id)) {
				...
				if (InputTextFilterCharacter(
				        &g, state, &c, callback, callback_user_data))
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
		...
		const int row_count_per_page = ImMax(
		    (int) ((inner_size.y - style.FramePadding.y) / g.FontSize), 1);
		...
		const bool is_redo =
		    (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Y, f_repeat, id) ||
		     Shortcut(
		         ImGuiMod_Ctrl | ImGuiMod_Shift | ImGuiKey_Z, f_repeat, id)) &&
		    !is_readonly && is_undoable;
		...
		const bool is_shift_enter =
		    Shortcut(ImGuiMod_Shift | ImGuiKey_Enter, f_repeat, id) ||
		...
		if (IsKeyPressed(ImGuiKey_LeftArrow)) {
			...
			if (!is_new_line) {
				...
				if (io.ConfigInputTextEnterKeepActive && !is_multiline &&
				    !is_ctrl_enter && !is_shift_enter) {
					// Queue reactivation, so that e.g.
					// IsItemDeactivatedAfterEdit() will work. (#9001)
					state->SelectAll();        // No need to scroll
					g.InputTextReactivateId =
					    id;        // Mark for reactivation on next frame
				}
			} else if (!is_readonly) {
				// Insert new line
				unsigned int c = '\n';
			...
			if (const char *clipboard = GetClipboardText()) {
				...
				for (const char *s = clipboard; *s != 0;) {
					...
					s += in_len;
					if (!InputTextFilterCharacter(
					        &g, state, &c, callback, callback_user_data, true))
						continue;
					char c_utf8[5];
	...
	if (g.ActiveId == id) {
		...
		if ((flags & (ImGuiInputTextFlags_CallbackCompletion |
			...
			// The reason we specify the usage semantic (Completion/History) is
			// that Completion needs to disable keyboard TABBING at the moment.
			...
			if (event_flag) {
				...
				// FIXME-OPT: Undo stack reconcile needs a backup of the data
				...
				if (callback_data.BufDirty ||
				    callback_data.CursorPos != state->Stb->cursor)
					state->CursorFollow = true;
				state->Stb->cursor = ImClamp(
				    callback_data.CursorPos, 0, callback_data.BufTextLen);
				state->Stb->select_start = ImClamp(
				    callback_data.SelectionStart, 0, callback_data.BufTextLen);
				state->Stb->select_end = ImClamp(
				    callback_data.SelectionEnd, 0, callback_data.BufTextLen);
				if (callback_data.BufDirty) {
					...
					IM_ASSERT(
					    ...
					    (int) ImStrlen(
					        callback_data.Buf));        // You need to maintain
					                                    // BufTextLen if you
					                                    // change the text!
					InputTextReconcileUndoState(state,
					                            state->CallbackTextBackup.Data,
					                            state->CallbackTextBackup.Size -
					                                1,
					                            callback_data.Buf,
					                            callback_data.BufTextLen);
					state->TextLen =
		...
		if (!is_readonly) {
			if (g.LastItemData.ItemFlags & ImGuiItemFlags_LiveEditOnInputText) {
				...
				// Apply on validation/deactivation, otherwise cancel out
				// previous apply attempts (e.g. revert)
				value_changed =
				    ((validated || clear_active_id || revert_edit) &&
				     strcmp(state->TextSrc, buf) != 0);
	...
	// InputTextDeactivateHook() for details) This is used when e.g. losing
	// focus or tabbing out into another InputText() which may already be using
	// the temp buffer.
	if (g.InputTextDeactivatedState.ID == id) {
		// The state only exists after an Edit. IsItemDeactivatedAfterEdit() is
		// not valid in every code path (see
		// "widgets_inputtext_status_noliveedit" test).
		if ((g.ActiveId != id && IsItemDeactivated()) ||
		    (g.ActiveId == id && (flags & ImGuiInputTextFlags_TempInput)))
			if (!is_readonly &&
				...
				value_changed = true;
				// IMGUI_DEBUG_LOG("InputText(): apply Deactivated data for
	...
	// Write back result to user buffer. This can currently only happen when
	// (g.ActiveId == id) or when just deactivated.
	// - As soon as the InputText() is active, our stored in-widget value gets
	// priority over any underlying modification of the user buffer.
	// - Make sure we always reapply the live buffer back to the input/user
	// buffer before clearing ActiveId, even thought strictly speaking
	//   it was not modified on this frame. This allows the user to use
	//   InputText() without maintaining any user-side storage. (PS: if you use
	//   this property together with ImGuiInputTextFlags_CallbackResize, you are
	//   at the risk of recreating a temporary
	//    allocated/string object every frame. Which in the grand scheme of
	//    scheme is nothing, but isn't dear imgui vibe).
	if (apply_new_text != NULL) {
		...
		if (is_resizable) {
			...
			callback_data.EventActivated =
			    (state != NULL && g.ActiveId == state->ID &&
		...
		ImStrncpy(
		    buf, apply_new_text, ImMin(apply_new_text_length + 1, buf_size));
	}
	...
	if (g.ActiveId == id && clear_active_id) {
		state->EditedBefore =
		    false;        // Data already applied: avoid
		                  // InputTextDeactivateHook() taking a record now or
		                  // later if same id is activated again without
		                  // editing.
		ClearActiveID();
	}
	...
	ImRect clip_rect(frame_bb.Min.x,
	                 frame_bb.Min.y,
	                 frame_bb.Min.x + inner_size.x,
	                 frame_bb.Min.y +
	                     inner_size.y);        // Not using frame_bb.Max because
	                                           // we have adjusted size
	if (is_multiline)
	...
	if (is_displaying_hint) {
		...
		if (render_cursor || render_selection || g.ActiveId == id)
			...
			buf_display_end =
			    NULL;        // Inactive multi-line: end of buffer will be
			                 // output by InputTextLineIndexBuild() special
			                 // strchr() path.
		else
	...
	if (is_multiline)
		CalcClipRectVisibleItemsY(clip_rect,
		                          draw_pos,
		                          g.FontSize,
		                          &line_visible_n0,
		                          &line_visible_n1);
	...
	const ImVec2 cursor_offset =
	    render_cursor && state ?
	        InputTextLineIndexGetPosOffset(g,
	                                       state,
	                                       line_index,
	                                       buf_display,
	                                       buf_display_end,
	...
	if (render_cursor || render_selection) {
		...
		if (render_selection) {
			...
			const float bg_offy_up =
			    is_multiline ?
			        0.0f :
			        -1.0f;        // FIXME-DPI: those offsets should be part of
			...
			for (int line_n = line_visible_n0; line_n < line_visible_n1;
				...
				if (text_selected_begin <= p_eol && text_selected_end > p_eol &&
				    !p_eol_is_wrap)
					rect_width += bg_eol_width;        // So we can see selected
					                                   // empty lines
				if (rect_width == 0.0f)
					continue;
	...
	if (g.ActiveId != id && (flags & ImGuiInputTextFlags_ElideLeft) &&
		...
		draw_pos.x = ImMin(draw_pos.x,
		                   frame_bb.Max.x - CalcTextSize(buf_display, NULL).x -
	...
	if (render_cursor) {
		...
		if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
			draw_window->DrawList->AddLineV(
			    cursor_screen_rect.Min.x,
			    cursor_screen_rect.Min.y,
			    cursor_screen_rect.Max.y,
		...
		if (!is_readonly && g.ActiveId == id) {
			ImGuiPlatformImeData *ime_data =
			    &g.PlatformImeData;        // (this is a public struct, passed
			                               // to io.Platform_SetImeDataFn()
			                               // handler)
	...
	if (is_multiline) {
		...
		g.NextItemData.ItemFlagsSet |=
		    (ImGuiItemFlags) ImGuiItemFlags_Inputable |
...
#	ifndef IMGUI_DISABLE_DEBUG_TOOLS
	...
	Text("TextLen: %d, Cursor: %d%s, Selection: %d..%d",
	     state->TextLen,
	     stb_state->cursor,
                       ...
                       ImGuiColorEditFlags flags)
{
	...
	if (!(flags & ImGuiColorEditFlags_DataTypeMask_))
		flags |=
		    (g.IO.ConfigColorEditFlags & ImGuiColorEditFlags_DataTypeMask_);
	...
	IM_ASSERT(ImIsPowerOfTwo(
	    flags & ImGuiColorEditFlags_DisplayMask_));        // Check that only 1
	                                                       // is selected
	IM_ASSERT(ImIsPowerOfTwo(
	    flags & ImGuiColorEditFlags_InputMask_));        // Check that only 1 is
	                                                     // selected
	...
	if ((flags & (ImGuiColorEditFlags_DisplayRGB |
		...
		if (InputText("##Text",
			...
			while (*p == '#' || ImCharIsBlankA(*p))
				p++;
			i[0] = i[1] = i[2] = 0;
			i[3] = 0xFF;        // alpha default to 255 is not parsed by scanf
			                    // (e.g. inputting #FFFFFF omitting alpha)
			int r;
			if (alpha)
				r = sscanf(p,
				           "%02X%02X%02X%02X",
	...
	if ((g.LastItemData.StatusFlags & ImGuiItemStatusFlags_HoveredRect) &&
		...
		if (const ImGuiPayload *payload =
			...
			memcpy((float *) col,
			       payload->Data,
			       sizeof(float) *
			           3);        // Preserve alpha if any //-V512 //-V1086
                         ...
                         const float        *ref_col)
{
	...
	const bool  is_readonly =
	    ((g.NextItemData.ItemFlagsSet | g.CurrentItemFlags) &
	...
	if (!(flags & ImGuiColorEditFlags_PickerMask_))
		flags |=
		    ((g.IO.ConfigColorEditFlags & ImGuiColorEditFlags_PickerMask_) ?
		         g.IO.ConfigColorEditFlags :
	...
	if (!(flags & ImGuiColorEditFlags_InputMask_))
		flags |= ((g.IO.ConfigColorEditFlags & ImGuiColorEditFlags_InputMask_) ?
		         ...
		         ImGuiColorEditFlags_InputMask_;
	IM_ASSERT(ImIsPowerOfTwo(
	    flags & ImGuiColorEditFlags_PickerMask_));        // Check that only 1
	                                                      // is selected
	IM_ASSERT(ImIsPowerOfTwo(
	    flags & ImGuiColorEditFlags_InputMask_));        // Check that only 1 is
	                                                     // selected
	...
	const bool triangle_rotate =
	    (flags & ImGuiColorEditFlags_PickerNoRotate) == 0;
	float  triangle_r  = wheel_r_inner - (int) (sv_picker_size * 0.027f);
	ImVec2 triangle_pa = triangle_rotate ?
	                         ImVec2(triangle_r, 0.0f) :
	                         ImVec2(triangle_r * +0.866f,
	                                triangle_r * +0.5f);        // Hue point.
	ImVec2 triangle_pb = triangle_rotate ?
	                         ImVec2(triangle_r * -0.5f, triangle_r * -0.866f) :
	                         ImVec2(0.0f, -triangle_r);        // Black point.
	ImVec2 triangle_pc = triangle_rotate ?
	                         ImVec2(triangle_r * -0.5f, triangle_r * +0.866f) :
	                         ImVec2(triangle_r * -0.866f,
	                                triangle_r * +0.5f);        // White point.
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel) {
		...
		if (IsItemActive() && !is_readonly) {
			...
			float cos_hue_angle =
			    triangle_rotate ? ImCos(-H * 2.0f * IM_PI) : 1.0f;
			float sin_hue_angle =
			    triangle_rotate ? ImSin(-H * 2.0f * IM_PI) : 0.0f;
		...
		if (IsItemActive() && !is_readonly) {
			...
			V = 1.0f - ImSaturate((io.MousePos.y - picker_pos.y) /
			                      (sv_picker_size - 1));
			ColorEditRestoreH(
			    col, &H);        // Greatly reduces hue jitter and reset to 0
			                     // when hue == 255 and color is rapidly
	...
	if (value_changed) {
		if (flags & ImGuiColorEditFlags_InputRGB) {
			...
			ColorConvertRGBtoHSV(R, G, B, H, S, V);
			ColorEditRestoreHS(
			    col, &H, &S, &V);        // Fix local Hue as display below will
			                             // use it immediately.
		} else if (flags & ImGuiColorEditFlags_InputHSV) {
			H = col[0];
			S = col[1];
	...
	ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(ImVec4(
	    R, G, B, style.Alpha));        // Important: this is still including the
	...
	int sv_cursor_segments = draw_list->_CalcCircleAutoSegmentCount(
	    sv_cursor_rad);        // Lock segment count so the +1 one matches
	                           // others.
	draw_list->AddCircleFilled(sv_cursor_pos,
	                           ...
	                           sv_cursor_segments);
	draw_list->AddCircle(
	    sv_cursor_pos, sv_cursor_rad + 1, col_midgrey, sv_cursor_segments);
	draw_list->AddCircle(
	    sv_cursor_pos, sv_cursor_rad, col_white, sv_cursor_segments);
                        ...
                        const ImVec2       &size_arg)
{
	...
	if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
		off = -0.75f;        // The border (using Col_FrameBg) tends to look off
		                     // when color is near-opaque and rounding is
	...
	if ((flags & ImGuiColorEditFlags_NoBorder) == 0) {
		if (g.Style.FrameBorderSize > 0.0f)
			...
			window->DrawList->AddRect(
			    ...
			    GetColorU32(ImGuiCol_FrameBg),
			    rounding,
			    1.0f * (float) (int) g.Style
			               ._MainScale);        // Color buttons are often in
...
#	ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
// This allowed passing 0 to set default, whereas NewFrame() will assert if one
// of missing. Abide to old logic.
...
void ImGui::ColorEditOptionsPopup(const float *col, ImGuiColorEditFlags flags)
{
	...
	if (BeginPopup("Copy")) {
		...
		if (!(flags & ImGuiColorEditFlags_NoAlpha)) {
			ImFormatString(
			    buf, IM_COUNTOF(buf), "#%02X%02X%02X%02X", cr, cg, cb, ca);
			if (Selectable(buf))
				SetClipboardText(buf);
		}
		EndPopup();
                                    ...
                                    ImGuiColorEditFlags flags)
{
	...
	if (allow_opt_picker) {
		ImVec2 picker_size(g.FontSize * 8,
		                   ImMax(g.FontSize * 8 - (GetFrameHeight() +
		                                           g.Style.ItemInnerSpacing.x),
		                         1.0f));        // FIXME: Picker size copied
		                                        // from main picker function
...
// The reason those two functions are not yet in public API is because I would
// like to design a more feature-full and generic API for this. They are
// otherwise function (cc: #3823, #9251, #7553, #6754, #5423, #2958, #2079,
// #1947, #1131, #722)
...
{
	// Leaf node always open a new tree/id scope. If you never use it, add
                             ...
                             const char        *label_end)
{
	...
	const float text_offset_y =
	    use_frame_padding ?
	        ...
	        window->DC.CurrLineTextBaseOffset;        // Latch before ItemSize
	                                                  // changes it
	const float text_width = g.FontSize + label_size.x +
	                         padding.x * 2;        // Include collapsing arrow
	...
	if (display_frame) {
		const float outer_extend =
		    IM_TRUNC(window->WindowPadding.x *
		             0.5f);        // Framed header expand a little outside of
		                           // current limits
		...
		if ((flags & ImGuiTreeNodeFlags_DrawLinesToNodes) &&
			...
			parent_data->DrawLinesToNodesY2 =
			    ImMax(parent_data->DrawLinesToNodesY2,
			          window->DC.CursorPos
			              .y);        // Don't need to aim to mid Y position as
			                          // we are clipped anyway.
	...
	const bool is_multi_select =
	    (g.LastItemData.ItemFlags & ImGuiItemFlags_IsMultiSelect) != 0;
	if (is_multi_select)        // We absolutely need to distinguish open vs
	                            // select so _OpenOnArrow comes by default
		...
		if (pressed && g.DragDropHoldJustPressedId != id) {
			...
			if (flags & ImGuiTreeNodeFlags_OpenOnArrow)
				toggled |=
				    ...
				    !g.NavHighlightItemUnderNav;        // Lightweight
				                                        // equivalent of
				                                        // IsMouseHoveringRect()
				                                        // since
				                                        // ButtonBehavior()
			...
			if (!is_open)        // When using Drag and Drop "hold to open" we
			                     // keep the node highlighted after opening, but
			                     // never close it again.
				toggled = true;
			else
				pressed = false;        // Cancel press so it doesn't trigger
				                        // selection.
		}
	...
	if (is_open && store_tree_node_stack_data)
		TreeNodeStoreStackData(
		    ...
		    text_pos.x -
		        text_offset_x);        // Call before TreePushOverrideID()
	if (is_open && !(flags & ImGuiTreeNodeFlags_NoTreePushOnOpen))
		TreePushOverrideID(id);        // Could use TreePush(label) but this
...
void ImGui::TreeNodeDrawLineToChildNode(const ImVec2 &target_pos)
{
	...
	if (rounding > 0.0f) {
		...
		window->DrawList->PathArcToFast(
		    ImVec2(x1, y - rounding), rounding, 6, 3);
		if (x1 < x2)
		...
		window->DrawList->AddLineH(
		    x1, x2, y, GetColorU32(ImGuiCol_TreeLines), g.Style.TreeLinesSize);
	}
}
...
void ImGui::TreeNodeDrawLineToTreePop(const ImGuiTreeNodeStackData *data)
{
	...
	if (data->TreeFlags & ImGuiTreeNodeFlags_DrawLinesFull) {
		...
		if (y2 + (g.Style.ItemSpacing.y + g.Style.TreeLinesRounding) <
		    y2_full)        // FIXME: threshold to use ToNodes Y2 instead of
		                    // Full Y2 when close by ItemSpacing.y
			y2 = y2_full;
	}
	...
	window->DrawList->AddLineV(
	    x, y1, y2, GetColorU32(ImGuiCol_TreeLines), g.Style.TreeLinesSize);
	if (data->DrawLinesTableColumn != -1)
...
void ImGui::TreePop()
{
	...
	IM_ASSERT(window->IDStack.Size >
	          1);        // There should always be 1 element in the IDStack
	                     // (pushed during window creation). If this triggers
	                     // you called TreePop/PopID too much.
	PopID();
}
                       ...
                       const ImVec2        &size_arg)
{
	...
	if (!is_visible)
		if (!is_multi_select || !g.BoxSelectState.UnclipMode ||
		    !g.BoxSelectState.UnclipRect.Overlaps(
		        bb))        // Extra layer of "no logic clip" for box-select
		                    // support (would be more overhead to add to
		                    // ItemAdd)
			return false;
	...
	if (g.ActiveId == id && g.ActiveIdIsJustActivated) {
		...
		g.ActiveIdWasSoleSelected =
		    was_selected &&
		    (!is_multi_select ||
		     g.CurrentMultiSelect->IsSoleOrUnknownSelectionSize);
	}
			...
			if (is_multi_select)
				nav_render_cursor_flags |=
				    ImGuiNavRenderCursorFlags_AlwaysDraw;        // Always show
				                                                 ...
				                                                 // rectangle
			RenderNavCursor(
			    bb, id, nav_render_cursor_flags, style.SelectableRounding);
		}
	}
...
#	ifdef IMGUI_DEBUG_BOXSELECT
	if (g.BoxSelectState.UnclipMode) {
		GetForegroundDrawList()->AddText(
		    pos, IM_COL32(255, 255, 0, 200), label, label_end);
	}
#	endif
    ...
    ImGui::GetTypingSelectRequest(ImGuiTypingSelectFlags flags)
{
	...
	for (ImWchar w : g.IO.InputQueueCharacters) {
		...
		if (w < 32 || (buffer_len == 0 && ImCharIsBlankW(w)) ||
		    (buffer_len + w_len >
		     buffer_max_len))        // Ignore leading blanks
			continue;
		char w_buf[5];
		...
		memcpy(
		    data->SearchBuffer + buffer_len, w_buf, w_len + 1);        // Append
		buffer_len += w_len;
		select_request = true;
	}
...
{
	if (req == NULL ||
	    req->SelectRequest == false)        // Support NULL parameter so both
	                                        // calls can be done from same spot.
		return -1;
	int idx = -1;
	if (req->SingleCharMode &&
		...
		idx = TypingSelectFindBestLeadingMatch(
		    req, items_count, get_item_name_func, user_data);
	if (idx != -1)
		SetNavCursorVisibleAfterMove();
	return idx;
}
    ...
    int   nav_item_idx)
{
	...
	for (int idx = 0; idx < items_count; idx++) {
		...
		if (first_match_idx == -1 &&
		    nav_item_idx == -1)        // Return first match immediately if we
		                               // don't have a nav_item_idx value.
			return idx;
		if (first_match_idx == -1)        // Record first match for wrapping.
			first_match_idx = idx;
		if (nav_item_idx == idx)        // Record that we encountering nav_item
		                                // so we can return next match.
	...
	for (int idx = 0; idx < items_count; idx++) {
		...
		const int   match_len =
		    ImStrimatchlen(req->SearchBuffer,
		                   req->SearchBuffer + req->SearchBufferLen,
		                   item_name);
                           ...
                           ImGuiMultiSelectFlags ms_flags)
{
	...
	// so "previous" rectangle is reprojected for current frame coordinates.
	...
	// IMGUI_DEBUG_LOG("StartPosRel (%.2f,%.2f) EndPosRel (%.2f,%.2f) ->
	// (%.2f,%.2f)\n", bs->StartPosRel.x, bs->StartPosRel.y, bs->EndPosRel.x,
	// bs->EndPosRel.y, WindowPosAbsToRel(window, g.IO.MousePos).x,
	// WindowPosAbsToRel(window, g.IO.MousePos).y);

	// Box-select 2D mode detects change of the rectangle.
	// Storing unclip rects which will be tested by widgets supporting
	// box-select. Always update rectangles when active (even if we don't use
	// them). To facilitate understanding this: enable IMGUI_DEBUG_BOXSELECT and
	// visualize all geometry.
	if (ms_flags & (ImGuiMultiSelectFlags_BoxSelect1d |
	                ImGuiMultiSelectFlags_BoxSelect2d)) {
		// For both sides, compute the area differing between Prev and Curr
		// rectangles.
		bs->UnclipRects[0] = bs->UnclipRects[1] =
		    ImRect(+FLT_MAX, +FLT_MAX, -FLT_MAX, -FLT_MAX);
		for (int side = 0; side < 2; side++) {
			ImVec2 d_min = (side == 0) ? ImMin(bs->BoxSelectRectCurr.Min,
			                                   bs->BoxSelectRectPrev.Min) :
			                             ImMin(bs->BoxSelectRectCurr.Max,
			                                   bs->BoxSelectRectPrev.Max);
			ImVec2 d_max = (side == 0) ? ImMax(bs->BoxSelectRectCurr.Min,
			                                   bs->BoxSelectRectPrev.Min) :
			                             ImMax(bs->BoxSelectRectCurr.Max,
			                                   bs->BoxSelectRectPrev.Max);
			if (d_min.x != d_max.x) {
		...
		// FIXME-OPT: When UnclipRect.Area() is much larger than the sum of
		// UnclipRects[0]/[1] Areas, widgets should ideally first use UnclipRect
		// as a first coarse cull layer + the individual ones as a second
		// validation.
		...
		if (!bs->UnclipRect.IsInverted() &&
		    (!window->ClipRect.Contains(bs->UnclipRect.Min) ||
		     !window->ClipRect.Contains(
		         bs->UnclipRect.Max)))        // !! Don't use Contains(ImRect)
			bs->UnclipMode = true;
		if (bs->UnclipMode && g.CurrentTable != NULL)
			TableApplyExternalUnclipRect(
			    g.CurrentTable,
			    bs->UnclipRect);        // No need submitting both
	}

#	ifdef IMGUI_DEBUG_BOXSELECT
	// GetForegroundDrawList()->AddRect(scope_rect.Min, scope_rect.Max,
	// IM_COL32(0, 255, 0, 200), 0.0f, 0, 4.0f);
		...
		for (ImRect &unclip_r : bs->UnclipRects)
			if (!unclip_r.IsInverted())
				GetForegroundDrawList()->AddRect(
				    unclip_r.Min,
				    unclip_r.Max,
				    bs->UnclipMode ? IM_COL32(255, 255, 0, 200) :
				                     IM_COL32(255, 0, 0, 200),
				    0.0f,
				    0,
				    4.0f);
		GetForegroundDrawList()->AddRect(bs->UnclipRect.Min,
		                                 bs->UnclipRect.Max,
		                                 bs->UnclipMode ?
...
static ImRect CalcScopeRect(ImGuiMultiSelectTempData *ms, ImGuiWindow *window)
{
	if (ms->Flags & ImGuiMultiSelectFlags_ScopeRect) {
		...
		// EndMultiSelect() only This probably doesn't work inside a table as
		// there are ample ambiguities related to exact time of calling
                                            ...
                                            int items_count)
{
	...
	if (ImGuiTable *table = g.CurrentTable) {
		if (!table->IsLayoutLocked)
			...
			TableEndCell(
			    table);        // This is currently safe to call multiple time.
			                   // If that properly is lost we can extract the
			                   // "save measurement" part of it.
	}
	...
	if (flags & ImGuiMultiSelectFlags_ScopeRect)
		window->DC.CursorMaxPos =
		    ms->ScopeRectMin;        // CalcScopeRect() for
		                             // ImGuiMultiSelectFlags_ScopeRect will
		                             // measure in EndMultiSelect().
	PushFocusScope(ms->FocusScopeId);
	ms->IsFocused = IsInNavFocusRoute(g.CurrentFocusScopeId);
	if (flags &
	    ImGuiMultiSelectFlags_ScopeWindow)        // Mark parent child window as
	                                              // navigable into, with
	                                              // highlight. Assume user will
	                                              // always submit interactive
	                                              // items.
	...
	if (g.NavJustMovedToId != 0 &&
		...
		if (ms->IsKeyboardSetRange)
			IM_ASSERT(storage->RangeSrcItem !=
			          ImGuiSelectionUserData_Invalid);        // Not ready ->
			                                                  // could clear?
	...
	if (flags & (ImGuiMultiSelectFlags_BoxSelect1d |
		...
		if (BeginBoxSelect(
		        CalcScopeRect(ms, window), window, ms->BoxSelectId, flags))
...
ImGuiMultiSelectIO *ImGui::EndMultiSelect()
{
	...
	// windows.
	bool scope_hovered = window->InnerRect.Contains(g.IO.MousePos) &&
	...
	if (ms->Flags & ImGuiMultiSelectFlags_NavWrapX) {
		IM_ASSERT(ms->Flags &
		          ImGuiMultiSelectFlags_ScopeWindow);        // Only supported
		                                                     // at window scope
    ...
    ImGuiSelectionUserData selection_user_data)
{
	...
	if (ImGuiMultiSelectTempData *ms = g.CurrentMultiSelect) {
		...
		// ms->PrevSubmittedItem = ms->CurrSubmittedItem; // Can't rely on
		// previous g.NextItemData.SelectionUserData because NextItemData is not
		// restored on nested multi-select. ms->CurrSubmittedItem =
		// selection_user_data;
	} else {
                                  ...
                                  ImGuiButtonFlags *p_button_flags)
{
	...
	if (p_button_flags != NULL) {
		...
		if (ms->Flags & ImGuiMultiSelectFlags_SelectOnClickAlways)
			...
			button_flags |= (!selected || (g.ActiveId == id &&
			                               g.ActiveIdHasBeenPressedBefore)) ?
			                    ImGuiButtonFlags_PressedOnClick :
			                    ImGuiButtonFlags_PressedOnClickRelease;
...
void ImGui::MultiSelectItemFooter(ImGuiID               id,
                                  bool                 *p_selected,
                                  bool                 *p_pressed,
                                  ImGuiMultiSelectFlags extra_flags)
{
	ImGuiContext &g      = *GImGui;
	ImGuiWindow  *window = g.CurrentWindow;
	...
	if (g.NavJustMovedToId == id) {
		if ((flags & ImGuiMultiSelectFlags_NoAutoSelect) == 0) {
			...
			if (is_shift)
				...
				apply_to_range_src =
				    true;        // Since if (pressed) {} main block is not
	...
	if (apply_to_range_src) {
		storage->RangeSrcItem  = item_data;
		storage->RangeSelected = selected;        // Will be updated at the end
	...
	if (ms->BoxSelectId != 0)
		if (ImGuiBoxSelectState *bs = GetBoxSelectState(ms->BoxSelectId)) {
			...
			if (!window->DC.NavIsScrollPushableX)        // FIXME: Rename to be
			                                             // more generic.
				if (ImGuiTable *table = g.CurrentTable)
					if (table->CurrentColumn != -1) {
						// FIXME: We cannot solely use current ClipRect as it
						// includes HostClipRect. However we account for
						// ClipRect being larger than current column (e.g. when
						// using SpanAllColumns) A more generic version would be
						// nice, but window->WorkRect.Min/Max exclude
						// CellPadding. (#7994, #9383)
						ImGuiTableColumn *column =
						    &table->Columns[table->CurrentColumn];
						float clip_min_x = (g.LastItemData.ItemFlags &
						                    ImGuiItemStatusFlags_HasClipRect) ?
						                       g.LastItemData.ClipRect.Min.x :
						                       window->ClipRect.Min.x;
						float clip_max_x = (g.LastItemData.ItemFlags &
						                    ImGuiItemStatusFlags_HasClipRect) ?
						                       g.LastItemData.ClipRect.Max.x :
						                       window->ClipRect.Max.x;
						if (clip_min_x !=
						    clip_max_x)        // When zero sized we expect that
						                       // bounds have been clamped and
						                       // thus are unreliable
						{
							item_rect.Min.x =
							    ImMax(item_rect.Min.x,
							          ImMin(column->MinX, clip_min_x));
							item_rect.Max.x =
							    ImMin(item_rect.Max.x,
							          ImMax(column->MaxX, clip_max_x));
						} else {
							item_rect.Min.x =
							    ImMax(item_rect.Min.x, column->MinX);
							item_rect.Max.x =
							    ImMin(item_rect.Max.x, column->MaxX);
						}
						// GetForegroundDrawList()->AddRect(item_rect.Min,
						// item_rect.Max, IM_COL32(255, 0, 255, 255));
					}
			const bool rect_overlap_curr =
			    bs->BoxSelectRectCurr.Overlaps(item_rect);
			...
			if ((rect_overlap_curr && !rect_overlap_prev && !selected) ||
				...
				if (storage->LastSelectionSize <= 0 &&
					...
					pressed = true;        // First item act as a pressed: code
					                       // below will emit selection request
					                       // and set NavId (whatever we emit
					                       // here will be overridden anyway)
					bs->IsStartedSetNavIdOnce = false;
				} else {
					selected = !selected;
					MultiSelectAddSetRange(
					    ms, selected, +1, item_data, item_data);
#	ifdef IMGUI_DEBUG_BOXSELECT
			...
			if (is_singleselect)
				...
				request_clear =
				    (flags & ImGuiMultiSelectFlags_NoAutoClearOnReselect) ?
			         ...
			         is_shift && !is_ctrl)
				request_clear =
				    true;        // With is_shift==false the RequestClear was
				                 // done in BeginIO, not necessary to do again.
			if (request_clear)
				MultiSelectAddSetAll(ms, false);
		}
...
{
	ImGuiSelectionRequest req = {ImGuiSelectionRequestType_SetAll,
	                             selected,
	                             0,
	                             ImGuiSelectionUserData_Invalid,
	                             ImGuiSelectionUserData_Invalid};
                                   ...
                                   ImGuiSelectionUserData    last_item)
{
	...
	// FIXME-OPT: Disabled on 2026/04/09 as this would break with any form of
	// coarse clipping that we don't know about (e.g. TableNextColumn() return
	// value). The low-hanging fruit would be to know that
	// ImGuiSelectionUserData are sequential indices, in which case we can
	// trivially compare PrevSubmittedItem + RangeDir == FirstItem. User can
...
#	ifndef IMGUI_DISABLE_DEBUG_TOOLS
	const bool is_active =
	    (storage->LastFrameActive >=
	     GetFrameCount() -
	         2);        // Note that fully clipped early out scrolling tables
    ...
    int                         size_before_amends)
{
	...
	if (selected && selection->Size != size_before_amends)
		storage
		    ->BuildSortByKey();        // When done selecting: sort everything
}
...
bool ImGui::BeginListBox(const char *label, const ImVec2 &size_arg)
{
	...
	ImVec2 size = ImTrunc(CalcItemSize(size_arg,
	                                   CalcItemWidth(),
...
void ImGui::EndListBox()
{
	...
	EndChild();
	EndGroup();        // This is only required to be able to do IsItemXXX query
	                   // on the whole ListBox including label
}
                    ...
                    int   height_in_items)
{
	...
	clipper.Begin(
	    items_count,
	    GetTextLineHeightWithSpacing());        // We know exactly our line
	                                            // height here so we pass it as
	                                            // a minor optimization, but
...
// Plot Histogram (the data provided _is_ histogram data. it doesn't compute the
...
bool ImGui::BeginMenuBar()
{
	...
	BeginGroup();        // Backup position on layer 0 // FIXME: Misleading to
	                     // use a group for that backup/restore
	PushID("##MenuBar");
...
void ImGui::EndMenuBar()
{
	...
	if (NavMoveRequestButNoResultYet() &&
		...
		if (nav_earliest_child->ParentWindow == window &&
			...
			if (g.NavCursorVisible) {
				g.NavCursorVisible =
				    false;        // Hide nav cursor for the current frame so we
				                  // don't see the intermediary selection. Will
				                  // be set again
	...
	window->DC.IdealMaxPos.x =
	    ImMax(window->DC.IdealMaxPos.x,
	          window->DC.CursorMaxPos.x -
	              window->Scroll.x);        // Convert ideal extents for
	                                        // scrolling layer equivalent.
	EndGroup();        // Restore position on layer 0 // FIXME: Misleading to
	                   // use a group for that backup/restore
	window->DC.LayoutType       = ImGuiLayoutType_Vertical;
	window->DC.IsSameLine       = false;
                                 ...
                                 ImGuiWindowFlags window_flags)
{
	...
	SetNextWindowViewport(
	    viewport->ID);        // Enforce viewport so we don't create our own
	                          // viewport when ImGuiConfigFlags_ViewportsNoMerge
	                          // is set.
...
void ImGui::EndMainMenuBar()
{
	...
	g.CurrentWindow->Flags |=
	    ImGuiWindowFlags_NoSavedSettings;        // Restore _NoSavedSettings
	                                             // (#8356)
...
bool ImGui::BeginMenuEx(const char *label, const char *icon, bool enabled)
{
	...
	if (g.MenusIdSubmittedThisFrame.contains(id)) {
		if (menu_is_open)
			menu_is_open = BeginPopupMenuEx(
			    id,
			    label,
			    window_flags);        // menu_is_open can be 'false' when the
			                          // popup is completely clipped (e.g. zero
			                          // size display)
		else
			g.NextWindowData
	...
	const ImGuiSelectableFlags selectable_flags =
	    ...
	    (ImGuiSelectableFlags) ImGuiSelectableFlags_SelectOnClick;
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		window->DC.CursorPos.x += IM_TRUNC(
		    ...
		    (-1.0f + 0.5f));        // -1 spacing to compensate the spacing
		                            // added when Selectable() did a SameLine().
		                            // It would also work to call SameLine()
	...
	// Once dragged, release ActiveId + key ownership. This is to allow the
	// idiom of mouse down a menu, dragging elsewhere, up on some other
	// MenuItem(). (#8233, #9394) Could move logic into lower-level
	// ImGuiButtonFlags_AutoReleaseActiveId +
	// ImGuiButtonFlags_AutoReleaseKeyOwner? Easier once we get rid of the
	// Selectable() middle-man here.
	if (g.ActiveId == id && g.HoveredId != id &&
	    g.ActiveIdSource == ImGuiInputSource_Mouse && IsMouseDragging(0)) {
		ClearActiveID();
	                                     ...
	                                     // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		if (g.HoveredWindow == window && child_menu_window != NULL) {
			...
			const float  pad_farmost_h =
			    ImClamp(ImFabs(ta.x - tb.x) * 0.30f,
			            ref_unit * 0.5f,
			...
			tb.y = ta.y + ImMax((tb.y - pad_farmost_h) - ta.y,
			                    -ref_unit *
			                        8.0f);        // Triangle has maximum height
	...
	if (menu_is_open) {
		...
		SetNextWindowPos(
		    popup_pos, ImGuiCond_Always);        // Note: misleading: the value
		                                         // will serve as reference for
		...
		menu_is_open = BeginPopupMenuEx(
		    id, label, window_flags);        // menu_is_open may be 'false' when
...
void ImGui::EndMenu()
{
	...
	ImGuiWindow *parent_window =
	    window->ParentWindow;        // Should always be != NULL is we passed
	                                 // assert.
                       ...
                       bool        enabled)
{
	...
	if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
		...
		PushStyleVarX(ImGuiStyleVar_ItemSpacing, style.ItemSpacing.x * 2.0f);
		pressed = Selectable(
		    "", selected, selectable_flags, ImVec2(label_size.x, 0.0f));
		PopStyleVar();
		...
		float checkmark_w = IM_TRUNC(
		    g.FontSize *
		    1.20f);        // FIXME: Always accounted for, even in a menu with
		                   // none, because 'selected' has no neutral setting.
		float min_w =
		    offsets->DeclColumns(icon_w,
		                         label_size.x,
		                         shortcut_w,
		...
		pressed =
		    Selectable("",
		               ...
		               ImVec2(min_w, label_size.y));
		if (g.LastItemData.StatusFlags & ImGuiItemStatusFlags_Visible) {
			RenderText(text_pos + ImVec2(offsets->OffsetLabel, 0.0f),
			           label,
			           label_end,
			           false);
			if (icon_w > 0.0f)
			...
			if (selected)
				RenderCheckMark(window->DrawList,
				                text_pos +
				                    ImVec2(offsets->OffsetMark + stretch_w +
				                               g.FontSize * 0.40f,
...
{
	if (MenuItemEx(
	        label, NULL, shortcut, p_selected ? *p_selected : false, enabled)) {
		if (p_selected)
			*p_selected = !*p_selected;
		return true;
	}
...
static void ImGui::TabBarLayout(ImGuiTabBar *tab_bar)
{
	...
	if (scroll_to_tab_id != 0)
		TabBarScrollToTab(tab_bar, scroll_to_tab_id, sections);
	         ...
	         IsMouseHoveringRect(
	             tab_bar->BarRect.Min, tab_bar->BarRect.Max, true) &&
                         ...
                         ImGuiWindow      *window)
{
	...
	IM_ASSERT(
	    g.CurrentTabBar !=
	    tab_bar);        // Can't work while the tab bar is active as our tab
	                     // doesn't have an X offset yet, in theory we
	                     // could/should test something like
	                     // (tab_bar->CurrFrameVisible < g.FrameCount) but we'd
	                     // need to solve why triggers the commented early-out
	                     // assert in BeginTabBarEx() (probably dock node going
	                     // from implicit to explicit in same frame)

	if (!window->HasCloseButton)
		tab_flags |=
		    ImGuiTabItemFlags_NoCloseButton;        // Set _NoCloseButton
	...
	new_tab.LastFrameVisible =
	    tab_bar->CurrFrameVisible;        // Required so BeginTabBar() doesn't
	                                      // ditch the tab
	if (new_tab.LastFrameVisible == -1)
		new_tab.LastFrameVisible = g.FrameCount - 1;
	new_tab.Window = window;        // Required so tab bar layout can compute
	                                // the tab width before tab submission
                                     ...
                                     ImGuiTabBarSection *sections)
{
	...
	float         margin =
	    g.FontSize *
	    1.0f;        // When to scroll to make Tab N+1 visible always make a bit
	                 // of N visible to suggest more scrolling area (since we
                      ...
                      ImGuiWindow      *docked_window)
{
	...
	if (!is_tab_button) {
		if (tab_appearing &&
			...
			if (!tab_bar_appearing || tab_bar->SelectedTabId == 0)
				TabBarQueueFocus(tab_bar,
				                 tab);        // New tabs gets activated
	...
	if (want_clip_rect)
		PushClipRect(ImVec2(ImClamp(bb.Min.x,
		                            tab_bar->ScrollingRectMinX,
		                            tab_bar->ScrollingRectMaxX),
		                    bb.Min.y - 1),
	...
	ImGuiButtonFlags button_flags =
	    ((ImGuiButtonFlags) (is_tab_button ?
	                             ImGuiButtonFlags_PressedOnClickRelease :
	                             ImGuiButtonFlags_PressedOnClick) |
	     ImGuiButtonFlags_AllowOverlap);
	if (g.DragDropActive &&
	    !g.DragDropPayload.IsDataType(
	        IMGUI_PAYLOAD_TYPE_WINDOW))        // FIXME: May be an opt-in
	                                           // property of the payload to
	                                           // disable this
		button_flags |= ImGuiButtonFlags_PressedOnDragDropHold;
                              ...
                              ImU32             col)
{
	...
	const float rounding =
	    ImMax(0.0f,
	          ImMin((flags & ImGuiTabItemFlags_Button) ? g.Style.FrameRounding :
	                                                     g.Style.TabRounding,
	                width * 0.5f - 1.0f));
	const float y1 =
	    bb.Min.y + 1.0f;        // Leave a bit of room in title bars.
	...
	if (g.Style.TabBorderSize > 0.0f) {
		...
		draw_list->PathStroke(GetColorU32(ImGuiCol_Border),
...
#	endif
	...
	bool is_hovered =
	    ...
	    g.ActiveId == tab_id ||
	    g.ActiveId ==
	        close_button_id;        // Any interaction account for this too.
	...
	if (unsaved_marker_visible) {
		...
		RenderBullet(
		    draw_list, bullet_pos, GetColorU32(ImGuiCol_UnsavedMarker));
