...
//    | TableApplyQueuedRequests()              - apply queued
...
//    columns) from their respective width | - TableSetColumnDisplayOrder() -
//    apply reordering a column | TableSetupDrawChannels()                -
//    setup ImDrawList channels | TableUpdateBorders()                    -
//    detect hovering columns for resize, ahead of contents submission |
//    TableBeginContextMenuPopup() | - TableDrawDefaultContextMenu()         -
...
static const int TABLE_DRAW_CHANNEL_NOCLIP =
    2;        // When using ImGuiTableFlags_NoClip (this becomes the last
              // visible channel)
static const float TABLE_BORDER_SIZE =
    1.0f;        // FIXME-TABLE: Currently hard-coded because of clipping
                 // assumptions with outer borders rendering.
                       ...
                       float           inner_width)
{
	...
	return BeginTableEx(
	    str_id, id, columns_count, flags, outer_size, inner_width);
}
                         ...
                         float           inner_width)
{
	...
	if (outer_window
	        ->SkipItems)        // Consistent with other tables + beneficial
	...
	const ImVec2 actual_outer_size = ImTrunc(CalcItemSize(
	    outer_size,
	    ImMax(avail_size.x, IMGUI_WINDOW_HARD_MIN_SIZE),
	...
	temp_data->ReconcileColumnsRequests.resize(
	    0);        // FIXME: Use shrink(0) everywhere?
	...
	if (use_child_window) {
		...
		table->InnerWindow->DC
		    .TreeDepth++;        // This is designed to always linking
		                         // ImGuiTreeNodeFlags_DrawLines linking across
		                         // a table
	}
	...
	if (instance_no > 0)
		PushOverrideID(
		    instance_id);        // FIXME: Somehow this is not resolved by
		                         // stack-tool, even tho GetIDWithSeed()
	...
	if (inner_window != outer_window) {
		...
		if (flags & ImGuiTableFlags_BordersOuterV) {
			table->HostClipRect.Min.x =
			    ImMin(table->HostClipRect.Min.x + border_size,
			          table->HostClipRect.Max.x);
			if (inner_window->DecoOuterSizeX2 == 0.0f)
				table->HostClipRect.Max.x =
				    ImMax(table->HostClipRect.Max.x - border_size,
				          table->HostClipRect.Min.x);
		}
		if (flags & ImGuiTableFlags_BordersOuterH) {
			table->HostClipRect.Min.y =
			    ImMin(table->HostClipRect.Min.y + border_size,
			          table->HostClipRect.Max.y);
			if (inner_window->DecoOuterSizeY2 == 0.0f)
				table->HostClipRect.Max.y =
				    ImMax(table->HostClipRect.Max.y - border_size,
				          table->HostClipRect.Min.y);
		}
	}
	...
	if (inner_window !=
	    outer_window)        // So EndChild() within the inner window can
	...
	if (old_columns_count != 0 && old_columns_count != columns_count) {
		...
		temp_data->OldColumnsRawData =
		    table->RawData;        // Freed during layout
	...
	if (table->IsInitializing) {
		...
		table->IsSettingsDirty =
		    true;        // Records itself into .ini file even when in default
		                 // state (#7934)
		table->IsReconcileMode    = false;
		table->InstanceInteracted = -1;
		table->ContextPopupColumn = -1;
		table->ReorderColumn      = table->ReorderColumnDstOrder =
		    table->ResizedColumn = table->LastResizedColumn = -1;
		table->AutoFitSingleColumn                          = -1;
		table->HoveredColumnBody = table->HoveredColumnBorder = -1;
		for (int n = 0; n < columns_count; n++) {
			ImGuiTableColumn *column = &table->Columns[n];
			if (temp_data->OldColumnsData.Data &&
			    n < temp_data->OldColumnsData.size()) {
...
void ImGui::TableApplyQueuedRequests(ImGuiTable *table)
{
	...
	//// Note: we don't clear ReorderColumn after handling the request (FIXME:
	///clarify why or add a test).
	if (table->InstanceCurrent == 0) {
		...
		if (table->ReorderColumn != -1 && table->ReorderColumnDstOrder != -1) {
			TableSetColumnDisplayOrder(
			    table, table->ReorderColumn, table->ReorderColumnDstOrder);
...
// Apply immediately. See TableQueueSetColumnDisplayOrder() for additional
// checks/constraints.
void ImGui::TableSetColumnDisplayOrder(ImGuiTable *table,
                                       int         column_n,
                                       int         dst_order)
{
	...
	// FIXME-OPT: If this is called multiple times we'd effectively have a
	// O(N^2) thing going on.
	for (int n = 0; n < table->ColumnsCount; n++)
		table->DisplayOrderToIndex[table->Columns[n].DisplayOrder] =
		    (ImGuiTableColumnIdx) n;
	table->IsSettingsDirty = true;
}

static int TableGetMaxDisplayOrderAllowed(ImGuiTable *table,
                                          int         src_order,
                                          int         dst_order)
{
	...
	// TableSetupScrollFreeze() enforce a display order range for frozen
	// columns. Reordering across the frozen column barrier is illegal and will
	// be undone.
	if (table->FreezeColumnsRequest > 0)
		dst_order =
		    (src_order < table->FreezeColumnsRequest) ?
		        ImMin(dst_order, (int) table->FreezeColumnsRequest - 1) :
	...
	for (int order_n = src_order;
	     (src_order < dst_order && order_n <= dst_order) ||
	     (dst_order < src_order && order_n >= dst_order);
	     order_n += reorder_dir)
		if (table->Columns[table->DisplayOrderToIndex[order_n]].Flags &
		    ImGuiTableColumnFlags_NoReorder) {
			dst_order =
			    (order_n == src_order) ? src_order : order_n - reorder_dir;
			break;
		}
	return dst_order;
}
...
void ImGui::TableQueueSetColumnDisplayOrder(ImGuiTable *table,
                                            int         column_n,
                                            int         dst_order)
{
	...
	if (table->IsLayoutLocked &&
	    dst_order ==
	        src_order)        // We allow calling the function before layout w/
	                          // reconcile so don't early out.
		return;
                                  ...
                                  ImGuiTableColumnFlags flags_in)
{
	...
	if ((flags & ImGuiTableColumnFlags_WidthMask_) == 0) {
		...
		IM_ASSERT(ImIsPowerOfTwo(
		    flags &
		    ImGuiTableColumnFlags_WidthMask_));        // Check that only 1 of
		                                               // each set is used.
	}
...
void ImGui::TableUpdateLayout(ImGuiTable *table)
{
	...
	if (table->IsInitializing || table->IsSettingsRequestLoad) {
		for (ImGuiTableColumn &column : table->Columns) {
			...
			if (table->IsSettingsRequestLoad)
				init_flags =
				    column.IsLoadedSettings ? ~table->SettingsLoadedFlags : ~0;
			else
				init_flags = column.IsJustCreated ? ~0 : 0;
			TableInitColumnDefaults(table, &column, init_flags);
		}
		TableFixDisplayOrder(table);        // Call even for non _Reorderable
		                                    // table as we loaded .ini data.
	...
	if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit) {
		...
		IMGUI_DEBUG_LOG_TABLE(
		    "[table] 0x%08X RefScale %.3f -> %.3f, scaling width by %.3f\n",
		    table->ID,
		    table->RefScale,
		    new_ref_scale_unit,
		    scale_factor);
	...
	int count_fixed =
	    0;        // Number of columns that have fixed sizing policies
	int count_stretch =
	...
	const float width_removed =
	    (table->HasScrollbarYPrev && !table->InnerWindow->ScrollbarY) ?
	        ...
	        0.0f;        // To synchronize decoration width of synced tables
	...
	if (width_remaining_for_stretched_columns >= 1.0f &&
		...
		for (int order_n = columns_count - 1;
...
// When starting a BeginMultiSelect() after table has been layout we update
// IsRequestOutput fields.
void ImGui::TableApplyExternalUnclipRect(ImGuiTable *table, ImRect &rect)
{
	...
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
		...
		if (!column->IsRequestOutput)
			if (rect.Overlaps(ImRect(column->MinX,
...
void ImGui::TableUpdateBorders(ImGuiTable *table)
{
	...
	const float hit_y2_body = ImMax(table->OuterRect.Max.y,
...
void ImGui::EndTable()
{
	...
	const float inner_content_max_y =
	    ImCeil(table->RowPosY2);        // Rounding final position is important
	                                    // as we currently don't round row
	                                    // height ('Demo->Tables->Outer Size'
	                                    // demo uses non-integer heights)
	IM_ASSERT(table->RowPosY2 == inner_window->DC.CursorPos.y);
	if (inner_window != outer_window)
		inner_window->DC.CursorMaxPos.y = inner_content_max_y;
	else if (!(flags & ImGuiTableFlags_NoHostExtendY))
		table->OuterRect.Max.y = table->InnerRect.Max.y = ImMax(
		    table->OuterRect.Max.y,
...
#	endif
	...
	if (inner_window != outer_window) {
		...
		inner_window->DC.NavLayersActiveMask |=
		    1 << table->NavLayer;        // So empty table don't appear to
	...
	if (table->Flags & ImGuiTableFlags_NoHostExtendX) {
		...
		// "table_reported_size_outer" equivalent Y block, #9352
		// - FIXME-TABLE: Would make sense to pre-compute expected scrollbar
		// visibility/sizes to generally save a frame of feedback? See broken
		// test in 'table_reported_size_outer'
		const float outer_content_max_x =
		    table->OuterRect.Min.x + table->ColumnsAutoFitWidth;
		const float decoration_size =
		    table->TempData->AngledHeadersExtraWidth +
		    ((inner_window != outer_window) ? inner_window->ScrollbarSizes.x :
		                                      0.0f);
		outer_window->DC.IdealMaxPos.x = ImMax(
		    outer_window->DC.IdealMaxPos.x,
		    outer_content_max_x + decoration_size - temp_data->UserOuterSize.x);
		outer_window->DC.CursorMaxPos.x =
		    ImMax(backup_outer_max_pos.x,
		          ImMin(table->OuterRect.Max.x,
		                outer_content_max_x + decoration_size));
	} else {
		outer_window->DC.CursorMaxPos.x =
	...
	if (temp_data->UserOuterSize.y <= 0.0f) {
		...
		const float outer_content_size_y =
		    (inner_window == outer_window) ?
		        (inner_content_max_y - table->InnerRect.Min.y) :
		        (inner_content_max_y - inner_window->DC.CursorStartPos.y);
		const float outer_content_max_y =
		    table->OuterRect.Min.y + outer_content_size_y;
		const float decoration_size =
		    (inner_window != outer_window ? inner_window->ScrollbarSizes.y :
		                                    0.0f);
		outer_window->DC.IdealMaxPos.y = ImMax(
		    outer_window->DC.IdealMaxPos.y,
		    outer_content_max_y + decoration_size - temp_data->UserOuterSize.y);
		outer_window->DC.CursorMaxPos.y =
		    ImMax(backup_outer_max_pos.y,
		          ImMin(table->OuterRect.Max.y,
		                outer_content_max_y + decoration_size));
	} else {
...
void ImGui::TableInitColumnDefaults(ImGuiTable           *table,
                                    ImGuiTableColumn     *column,
                                    ImGuiTableColumnFlags init_mask)
{
	...
	if (init_mask & ImGuiTableFlags_Resizable) {
		...
		if (init_width_or_weight >
		    0.0f)        // Disable auto-fit if an explicit width/weight has
		                 // been specified
			column->AutoFitQueue = 0x00;
	}
	if (init_mask & ImGuiTableFlags_Reorderable)
		column->DisplayOrder =
		    (ImGuiTableColumnIdx) ((table->Flags &
		                            ImGuiTableFlags_Reorderable) ?
		                               -1 :
		                               table->Columns.index_from_ptr(column));
...
static void TableSetupColumnApply(ImGuiTable           *table,
                                  int                   idx,
                                  ImGuiID               id,
                                  ImS16                 name_offset,
                             ...
                             ImGuiID               user_data)
{
	...
	IM_ASSERT_USER_ERROR_RET(table->IsLayoutLocked == false,
	                         "TableSetupColumn(): need to call before first "
	                         "row!");        // Table layout is locked when
	                                         // submitting a row or when calling
	                                         // BeginMultiSelect() with
	                                         // box-select.
	...
	const ImGuiID column_id =
	    (label != NULL && label[0] != 0) ? ImHashStr(label) : 0;

	// When ID changed or a column moved: defer the request until layout where
	// we will process full reconcile.
	...
	if (table->IsReconcileMode == false && column->ID != column_id) {
		...
		table->TempData->ReconcileColumnsRequests.reserve(table->ColumnsCount -
		                                                  column_idx);
	}

	// Fast/common path
	if (table->IsReconcileMode == false) {
		TableSetupColumnApply(table,
		                      column_idx,
		                      column_id,
		                      name_offset,
		                      flags,
		...
		if (column_idx < table->TempData->OldColumnsData.size())
			table->TempData->OldColumnsData[column_idx].IsNeedReconcileSrc =
			    false;
		return;
	}

	// Reconcile path: defer applying data to TableUpdateLayout() ->
	// TableReconcileMovedColumns() -> TableSetupColumnApply(). On column count
	// change: search the previous columns:
	// - the live Columns[] array is truncated on shrink, so a moved column may
	// only exist in old data.
	// - the live Columns[] array may already be partially reset.
	table->TempData->ReconcileColumnsRequests.push_back(
	    ImGuiTableReconcileColumnData());
	ImGuiTableReconcileColumnData &reconcile_data =
	    table->TempData->ReconcileColumnsRequests.back();
	...
	column->NameOffset =
	    name_offset;        // Allow TableGetColumnName() to work before layout
	column->IsNeedReconcileSrc = column->IsNeedReconcileDst = true;
}

// NB: This was written to be similar to the logic in
...
void ImGui::TableReconcileColumns(ImGuiTable *table)
{
	...
	ImSpan<ImGuiTableColumn> &src_columns =
	    table->TempData->OldColumnsData.empty() ?
	        table->Columns :
	        table->TempData->OldColumnsData;

	// Find matches for named columns.
	int                                      matches = 0;
	ImVector<ImGuiTableReconcileColumnData> &reconcile_requests =
	    temp_data->ReconcileColumnsRequests;
	for (ImGuiTableReconcileColumnData &reconcile_data : reconcile_requests)
		if (reconcile_data.ID != 0)
			for (ImGuiTableColumn &src_column : src_columns)
				if (src_column.ID == reconcile_data.ID &&
				    src_column.IsNeedReconcileSrc) {
					ImGuiTableColumn &dst_column =
					    dst_columns[reconcile_data.ColumnNewIdx];
					IM_ASSERT(src_column.IsNeedReconcileSrc &&
					          dst_column.IsNeedReconcileDst);
					src_column.IsNeedReconcileSrc =
					    dst_column.IsNeedReconcileDst = false;
					reconcile_data.ColumnOldIdx =
					    (ImGuiTableColumnIdx) src_columns.index_from_ptr(
					        &src_column);
	...
	if (matches != reconcile_requests.Size)
		for (ImGuiTableColumn &src_column : src_columns) {
			if (!src_column.IsNeedReconcileSrc)
				continue;
			while (dst_idx < reconcile_requests.Size &&
			       reconcile_requests[dst_idx].ColumnOldIdx !=
			           (ImGuiTableColumnIdx) -1)
				dst_idx++;
			if (dst_idx == reconcile_requests.Size)
				break;
			ImGuiTableReconcileColumnData &reconcile_data =
			    reconcile_requests[dst_idx];
			ImGuiTableColumn &dst_column =
			    dst_columns[reconcile_data.ColumnNewIdx];
			IM_ASSERT(src_column.IsNeedReconcileSrc &&
			          dst_column.IsNeedReconcileDst);
			src_column.IsNeedReconcileSrc = dst_column.IsNeedReconcileDst =
			    false;
			reconcile_data.ColumnOldIdx =
			    (ImGuiTableColumnIdx) src_columns.index_from_ptr(&src_column);
			reconcile_data.ColumnOldData = src_column;
		}

	// Apply in the final pass. Because it is possible that src_columns ==
	// table->Columns we went through a temporary copy.
	for (ImGuiTableReconcileColumnData &reconcile_data : reconcile_requests) {
		table->Columns[reconcile_data.ColumnNewIdx] =
		    reconcile_data
		        .ColumnOldData;        // When old column was not found clear
		                               // anyway with default-constructed data
		                               // (which will set IsJustCreated=true)
		TableSetupColumnApply(table,
		                      reconcile_data.ColumnNewIdx,
		                      reconcile_data.ID,
		                      reconcile_data.NameOffset,
		                      reconcile_data.Flags,
		                      reconcile_data.InitWidthOrWeight,
		                      reconcile_data.UserData);
		IMGUI_DEBUG_LOG_TABLE(
		    "[table] - old %d -> new %d \"%s\"\n",
		    reconcile_data.ColumnOldIdx,
		    reconcile_data.ColumnNewIdx,
		    TableGetColumnName(
		        table,
		        reconcile_data.ColumnNewIdx));        // Log at the end so
		                                              // NameOffset was copied.
	}
...
void ImGui::TableSetColumnEnabled(int column_n, bool enabled)
{
	...
	IM_ASSERT_USER_ERROR_RET(
	    table != NULL, "Call should only be done while in BeginTable() scope!");
	IM_ASSERT(table->Flags &
	          ImGuiTableFlags_Hideable);        // See comments above
	if (column_n < 0)
		column_n = table->CurrentColumn;
	IM_ASSERT(column_n >= 0 && column_n < table->ColumnsCount);
	ImGuiTableColumn *column       = &table->Columns[column_n];
	...
	window->DC.CursorPosPrevLine =
	    ImVec2(window->DC.CursorPos.x,
	           window->DC.CursorPos.y +
	               table->RowCellPaddingY);        // This allows users to call
	                                               // SameLine() to share
	                                               // LineSize between columns.
...
void ImGui::TableEndRow(ImGuiTable *table)
{
	...
	if (is_visible) {
		...
		if (bg_col0 || bg_col1) {
			ImRect row_rect(
			    table->WorkRect.Min.x, bg_y1, table->WorkRect.Max.x, bg_y2);
		...
		if (top_border_col && bg_y1 >= table->BgClipRect.Min.y &&
			...
			window->DrawList->AddLineH(table->BorderX1,
			                           table->BorderX2,
			                           bg_y1,
			                           top_border_col,
			                           border_size);
...
bool ImGui::TableSetColumnIndex(int column_n)
{
	...
	if (table->CurrentColumn != column_n) {
		...
		IM_ASSERT_USER_ERROR_RETV(
		    column_n >= 0 && column_n < table->ColumnsCount,
		    false,
...
void ImGui::TableEndCell(ImGuiTable *table)
{
	...
	if (table->RowFlags & ImGuiTableRowFlags_Headers)
		p_max_pos_x =
		    &column
		         ->ContentMaxXHeadersUsed;        // Useful in case user submit
		                                          // contents in header row that
		                                          // is not a TableHeader() call
	else
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
	...
	const float max_width =
	    ImMax(min_width,
	          column_0->WidthMax);        // Don't use TableCalcMaxColumnWidth()
	                                      // here as it would rely on MinX from
	                                      // last instance (#7933)
...
void ImGui::TableSetColumnWidthAutoAll(ImGuiTable *table)
{
	for (int column_n = 0; column_n < table->ColumnsCount; column_n++) {
		...
		if (!column->IsEnabled &&
		    !(column->Flags &
		      ImGuiTableColumnFlags_WidthStretch))        // Cannot reset weight
		                                                  // of hidden stretch
		                                                  // column
			continue;
		column->CannotSkipItemsQueue = (1 << 0);
		...
		for (int merge_group_sub_n = 0;
			...
			if (!(column->Flags & ImGuiTableColumnFlags_NoClip)) {
				...
				if (!has_freeze_v)
					...
					content_max_x = ImMax(
					    column->ContentMaxXFrozen,
					    column->ContentMaxXHeadersUsed);        // Row freeze:
					                                            // use width
					                                            // before freeze
				else
					content_max_x =
					    column
					        ->ContentMaxXUnfrozen;        // Row freeze: use
					                                      // width after freeze
...
#	endif
	...
	if (merge_group_mask != 0) {
		...
		ImBitArraySetBitRange(
		    remaining_mask, LEADING_DRAW_CHANNELS, splitter->_Count);
...
static ImU32
    TableGetColumnBorderCol(ImGuiTable *table, int order_n, int column_n)
{
...
void ImGui::TableDrawBorders(ImGuiTable *table)
{
	...
	if (table->Flags & ImGuiTableFlags_BordersInnerV) {
		for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
			...
			if (column->MaxX <=
			    column->ClipRect.Min
			        .x)        // FIXME-TABLE FIXME-STYLE: Assume BorderSize==1,
			                   // this is problematic if we want to increase the
			                   // border size..
				continue;
	...
	if (table->Flags & ImGuiTableFlags_BordersOuter) {
		...
		if ((table->Flags & ImGuiTableFlags_BordersOuter) ==
			...
			inner_drawlist->AddLineV(outer_border.Min.x,
			                         outer_border.Min.y,
			                         outer_border.Max.y,
			                         outer_col,
			                         border_size);
			inner_drawlist->AddLineV(outer_border.Max.x,
			                         outer_border.Min.y,
			                         outer_border.Max.y,
			                         outer_col,
			                         border_size);
		} else if (table->Flags & ImGuiTableFlags_BordersOuterH) {
			inner_drawlist->AddLineH(outer_border.Min.x,
			                         outer_border.Max.x,
			                         outer_border.Min.y,
			                         outer_col,
			                         border_size);
			inner_drawlist->AddLineH(outer_border.Min.x,
			                         outer_border.Max.x,
			                         outer_border.Max.y - border_size,
			                         outer_col,
			                         border_size);
		}
	}
	if ((table->Flags & ImGuiTableFlags_BordersInnerH) &&
		...
		if (border_y >= table->BgClipRect.Min.y &&
			...
			inner_drawlist->AddLineH(table->BorderX1,
			                         table->BorderX2,
			                         border_y,
			                         table->BorderColorLight,
			                         border_size);
		...
		if (IM_BITARRAY_TESTBIT(table->EnabledMaskByIndex, column_n))
			if (table->Columns[column_n].Flags &
				...
				width =
				    ImMax(width,
				          CalcTextSize(
				              TableGetColumnName(table, column_n), NULL, true)
				              .x);
...
void ImGui::TableHeader(const char *label)
{
	...
	ItemSize(ImVec2(
	    0.0f, label_height));        // Don't declare unclipped width, it'll be
	...
	if (held && (table->Flags & ImGuiTableFlags_Reorderable) &&
		...
		// - While moving a column it will jump on the other side of the mouse,
		...
		// - The other constraints are enforced by
		// TableQueueSetColumnDisplayOrder() which might early out.
		table->InstanceInteracted = table->InstanceCurrent;
		if (g.IO.MouseDelta.x < 0.0f && g.IO.MousePos.x < cell_r.Min.x)
			if (ImGuiTableColumn *prev_column =
				...
				TableQueueSetColumnDisplayOrder(
				    table, column_n, prev_column->DisplayOrder);
		if (g.IO.MouseDelta.x > 0.0f && g.IO.MousePos.x > cell_r.Max.x)
			if (ImGuiTableColumn *next_column =
			        (column->NextEnabledColumn != -1) ?
			            ...
			            NULL)
				TableQueueSetColumnDisplayOrder(
				    table, column_n, next_column->DisplayOrder);
	}

	// Sort order arrow
	// - Only clip label for a visible arrow. Auto-fit still accounts for a
	// possible arrow, but when
	//   manually sized smaller we don't clip the label for the sake of an arrow
	//   that isn't displayed.
	const float ellipsis_max =
	    ImMax(cell_r.Max.x - (sort_arrow ? w_arrow + w_sort_text : 0.0f),
	          label_pos.x);
	if ((table->Flags & ImGuiTableFlags_Sortable) &&
	    !(column->Flags & ImGuiTableColumnFlags_NoSort)) {
		if (column->SortOrder != -1) {
		...
		if (pressed && table->ReorderColumn != column_n) {
			...
			TableSetColumnSortDirection(
			    column_n, sort_direction, g.IO.KeyShift);
		}
	}
...
void ImGui::TableAngledHeadersRow()
{
	...
	ImGuiTableInstanceData *table_instance =
	    TableGetInstanceData(table, table->InstanceCurrent);
	int highlight_column_n = (table->LastHeldHeaderColumn != -1) ?
	...
	for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
		if (IM_BITARRAY_TESTBIT(table->EnabledMaskByDisplayOrder, order_n)) {
			...
			if ((column->Flags & ImGuiTableColumnFlags_AngledHeader) ==
			    0)        // Note: can't rely on ImGuiTableColumnFlags_IsVisible
			              // test here.
				continue;
			ImGuiTableHeaderData request = {(ImGuiTableColumnIdx) column_n,
			                                col_text,
			                                col_header_bg,
                                    ...
                                    int                         data_count)
{
	...
	const ImVec2 header_angled_vector =
	    unit_right *
	    (row_height / -sin_a);        // vector from bottom-left to top-left,
	                                  // and from bottom-right to top-right

	// Declare row, override and draw our own background
	// FIXME-TABLE: Generally broken when overlapping frozen columns limit.
	TableNextRow(ImGuiTableRowFlags_Headers, row_height);
	TableNextColumn();
	const ImRect row_r(table->WorkRect.Min.x,
	...
	if (table->FreezeColumnsCount > 0)
		clip_rect_min_x = ImMax(
		    clip_rect_min_x,
		    table
		        ->Columns[table->DisplayOrderToIndex[table->FreezeColumnsCount -
		                                             1]]
		        .MaxX);
	TableSetBgColor(ImGuiTableBgTarget_RowBg0, 0);        // Cancel
	PushClipRect(table->BgClipRect.Min,
	...
	draw_list->AddRectFilled(
	    ...
	    GetColorU32(ImGuiCol_TableHeaderBg,
	                0.25f));        // FIXME-STYLE: Change row background with
	                                // an arbitrary color.
	...
	const float ascent_scaled =
	    g.FontBaked->Ascent *
	    g.FontBakedScale;        // FIXME: Standardize those scaling factors
	                             // better
		...
		for (int order_n = 0; order_n < data_count; order_n++) {
			...
			if (pass == 0) {
				...
				while (label_name < label_name_end) {
					...
					float clip_height =
					    ImMin(label_size.y,
					          column->ClipRect.Max.x - column->WorkMinX -
					              line_off_curr_x);
			...
			if (pass == 1) {
				// Draw border
				draw_list->AddLine(
				    bg_shape[0] - ImVec2(TABLE_BORDER_SIZE * 0.5f, 0.0f),
				    bg_shape[3] - ImVec2(TABLE_BORDER_SIZE * 0.5f, 0.0f),
				    TableGetColumnBorderCol(table, order_n, column_n),
				    TABLE_BORDER_SIZE);
			}
		}
...
// FIXME: Copied from MenuItem() for the purpose of being able to pass
// _SelectOnRelease (#9312)
static bool
    MenuItemForColumnReorder(const char *label, bool selected, bool enabled)
{
	...
	if (g.ActiveId == id)
		selectable_flags |=
		    ImGuiSelectableFlags_Highlight;        // Stays highlighted while
		                                           // dragging.
	const bool has_been_moved =
	    (g.ActiveId == id) &&
	    g.ActiveIdHasBeenEditedBefore;        // But disable toggling once
	                                          // moved.

	BeginDisabled(!enabled);        // Don't use ImGuiSelectableFlags_Disabled
	                                // so that Check mark is also affected.
	bool ret =
	    Selectable(
	        label, false, selectable_flags, ImVec2(min_w, label_size.y)) &&
	    !has_been_moved;        // Can't use IsMouseDragging(0) as button is
	                            // released already.
    ...
    ImGuiTable *table, ImGuiTableFlags flags_for_section_to_display)
{
	...
	ImGuiTableColumn *context_column =
	    (context_column_n != -1) ? &table->Columns[context_column_n] : NULL;

	// Sizing
	if (flags_for_section_to_display & ImGuiTableFlags_Resizable) {
		if (context_column != NULL) {
			const bool can_resize =
			    !(context_column->Flags & ImGuiTableColumnFlags_NoResize) &&
	...
	if (flags_for_section_to_display &
	    (ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable))
		if (BeginMenu(LocalizeGetMsg(ImGuiLocKey_TableReset))) {
			// if (MenuItem(LocalizeGetMsg(ImGuiLocKey_TableResetAll))) //
			// FIXME: Hiding because altering Sort Order + requesting auto-fit
			// simultaneously has a tendency to exhibit a sizing glitch.
			...
			if (flags_for_section_to_display & ImGuiTableFlags_Reorderable)
				if (MenuItem(
				        ...
				        NULL,
				        false,
				        !table->IsDefaultDisplayOrder))        // PS: cannot be
				                                               // hidden because
				                                               // it would mess
				                                               // with drag
				                                               // reordering.
			...
			if (flags_for_section_to_display & ImGuiTableFlags_Hideable)
				if (MenuItem(LocalizeGetMsg(ImGuiLocKey_TableResetVisibility),
				             NULL,
				             false,
...
#	endif
	...
	if (flags_for_section_to_display & ImGuiTableFlags_Hideable) {
		...
		// While reordering: we calculate min/max allowed range once here so we
		// can avoid a O(N log N) in the loop (because the query itself does a
		// sweep scan). This assume that reordering constraints output a single
		// range, otherwise would need to either call
		// TableGetMaxDisplayOrderAllowed() for each item below, or cache this
		// once per frame into columns.
		const bool is_reordering =
		    (g.ActiveId != 0 && g.ActiveIdWindow == g.CurrentWindow &&
		     table->ReorderColumn != -1 &&
		     g.ActiveIdHasBeenEditedBefore);        // FIXME: This is a bit of a
		                                            // hack.
		const int reorder_src_order =
		    is_reordering ? table->Columns[table->ReorderColumn].DisplayOrder :
		                    -1;
		const int reorder_min_order =
		    is_reordering ?
		        TableGetMaxDisplayOrderAllowed(table, reorder_src_order, 0) :
		        0;
		const int reorder_max_order =
		    is_reordering ?
		        TableGetMaxDisplayOrderAllowed(
		            table, reorder_src_order, table->ColumnsCount - 1) :
		...
		for (int order_n = 0; order_n < table->ColumnsCount; order_n++) {
			...
			bool menu_item_enabled =
			    (column->Flags & ImGuiTableColumnFlags_NoHide) ? false : true;
			if (column->IsUserEnabled && table->ColumnsEnabledCount <= 1)
				menu_item_enabled = false;
			if (is_reordering && (column->DisplayOrder < reorder_min_order ||
			                      column->DisplayOrder > reorder_max_order))
				menu_item_enabled = false;
			if (MenuItemForColumnReorder(
			        name, column->IsUserEnabled, menu_item_enabled))
				column->IsUserEnabledNextFrame = !column->IsUserEnabled;

			// Drag to reorder
			// FIXME: It is currently not possible to reorder columns marked
			// with ImGuiTableColumnFlags_NoHide.
			if (IsItemActive() && IsMouseDragging(0) &&
			    g.ActiveIdSource == ImGuiInputSource_Mouse &&
			    (table->Flags & ImGuiTableFlags_Reorderable)) {
				g.ActiveIdHasBeenEditedBefore =
				    true;        // Disable toggle in MenuItemForColumnReorder()
				                 // + start dimming to display allowed reorder
				                 // targets.
				table->ReorderColumn = (ImGuiTableColumnIdx) column_n;
				if (!IsItemHovered()) {
					int reorder_dir =
					    (g.IO.MousePos.y < (g.LastItemData.Rect.Min.y +
					                        g.LastItemData.Rect.Max.y) *
					                           0.5f) ?
					        -1 :
					        +1;
					float reorder_amount =
					    (reorder_dir < 0 ?
					         g.LastItemData.Rect.Min.y - g.IO.MousePos.y :
					         g.IO.MousePos.y - g.LastItemData.Rect.Max.y) /
					    g.LastItemData.Rect.GetHeight();
					int dst_order =
					    column->DisplayOrder +
					    (int) ImCeil(reorder_amount) *
					        reorder_dir;        // Estimated target order, will
					                            // be validated and clamped.
...
void ImGui::TableResetSettings(ImGuiTable *table)
{
	...
	table->SettingsLoadedFlags =
	    ImGuiTableFlags_None;        // Mark as nothing loaded so our
...
void ImGui::TableLoadSettings(ImGuiTable *table)
{
	...
	if (table->SettingsOffset == -1) {
		...
		if (settings->ColumnsCount !=
		    table->ColumnsCount)        // Allow settings if columns count
		                                // changed. We could otherwise decide to
		                                // return...
			table->IsSettingsDirty = true;
		table->SettingsOffset = g.SettingsTables.offset_from_ptr(settings);
	} else {
	...
	settings->LastUsedDate     = g.SessionDate;
	// TableUpdateLayout() will then call TableLoadSettingsForColumns() to apply
	// the data.
}
...
void ImGui::TableLoadSettingsForColumns(ImGuiTable *table)
{
	...
	table->SettingsLoadedFlags |=
	    ImGuiTableFlags_Reorderable;        // We handle above in code above.
	...
	int matches = 0;
	for (int n = 0; n < table->ColumnsCount; n++) {
		if (n >= settings->ColumnsCount ||
		    column_settings[n].ID != table->Columns[n].ID)
			break;
		TableLoadSettingsForColumn(
		    &table->Columns[n], &column_settings[n], settings->SaveFlags);
		matches++;
	}
	...
	for (ImGuiTableColumn &column : table->Columns)
		if (column.ID != 0 && !column.IsLoadedSettings)
			for (int n = settings_start_n; n < settings->ColumnsCount; n++)
				if (column_settings[n].ID == column.ID &&
				    !column_settings[n].IsLoaded) {
					TableLoadSettingsForColumn(
					    &column, &column_settings[n], settings->SaveFlags);
					matches++;
					break;
				}
	...
	for (int n = settings_start_n; n < settings->ColumnsCount; n++)
		if (!column_settings[n].IsLoaded) {
			while (dst_idx < table->ColumnsCount &&
			       table->Columns[dst_idx].IsLoadedSettings)
				dst_idx++;
			if (dst_idx >= table->ColumnsCount)
				break;
			TableLoadSettingsForColumn(&table->Columns[dst_idx],
			                           &column_settings[n],
			                           settings->SaveFlags);
			dst_idx++;
		}
...
void ImGui::TableLoadSettingsForColumn(
    ImGuiTableColumn         *column,
    ...
    ImGuiTableFlags           load_flags)
{
	...
	if (load_flags & ImGuiTableFlags_Reorderable)
		...
		column->DisplayOrder =
		    column_settings
		        ->Index;        // Because default depends on previous Index, we
		                        // need to set that up and cannot rely on
		                        // TableInitColumnDefaults()
	if ((load_flags & ImGuiTableFlags_Hideable) &&
...
struct ImGuiTableFixDisplayOrderColumnData {
	...
	ImGuiTable *Table;        // This is unfortunate but we don't have userdata
	                          // in qsort api.
};
                                                    ...
                                                    const void *rhs)
{
	...
	// Assume ImGuiTableColumnIdx == signed short, to turn -1 into a large value
	// so that it always sort after.
	const int order_delta =
	    ((unsigned short) table->Columns[lhs_idx].DisplayOrder -
	     (unsigned short) table->Columns[rhs_idx].DisplayOrder);
...
void ImGui::TableFixDisplayOrder(ImGuiTable *table)
{
	...
	g.TempBuffer.reserve(
	    (int) (sizeof(ImGuiTableFixDisplayOrderColumnData) *
	           table->ColumnsCount));        // FIXME: Maybe wrap those two
	                                         // lines as a helper.
...
static void TableSettingsHandler_Cleanup(ImGuiContext *ctx,
                                         ImGuiSettingsHandler *,
                                         ImGuiSettingsCleanupArgs *args)
{
	ImGuiContext &g = *ctx;
	for (int i = 0; i != g.Tables.GetMapSize(); i++)
		if (ImGuiTable *table = g.Tables.TryGetMapData(i))
	...
	for (ImGuiTableSettings *settings = g.SettingsTables.begin();
		...
		if (args->_DiscardOlderThanDate != 0 &&
		    settings->LastUsedDate.Unpack() < args->_DiscardOlderThanDate)
			settings->ID = 0;
		else if (args->DiscardWhenMissingDate && !is_valid)
			settings->ID = 0;
		else if (args->SetCurrentSessionDateToAll ||
		         (args->SetCurrentSessionDateWhenMissingDate && !is_valid))
			settings->LastUsedDate = g.SessionDate;
	}
}
...
static void *TableSettingsHandler_ReadOpen(ImGuiContext *,
                                           ...
                                           const char *name)
{
	// FIXME: As topology changes are allowed, strictly speaking a >=
	// IMGUI_TABLE_MAX_COLUMNS tables stored in .ini file that was emitted with
	// a higher max count could still be meaningful in some unlikely cases. We
	// might want to use another MAX defined as
	// (1<<(sizeof(ImGuiTableColumnIdx)*8-1))-1.
	ImGuiID id            = 0;
	int     columns_count = 0;
                                          ...
                                          const char *line)
{
	...
	if (sscanf(line, "Column %d%n", &column_n, &r) == 1) {
		...
		if (sscanf(line, "UserID=0x%08X%n", (ImU32 *) &n, &r) == 1) {
			line = ImStrSkipBlank(line + r);
		}        // FIXME-LEGACY: Removed 2025/11/12, was never properly set.
                                          ...
                                          ImGuiTextBuffer      *buf)
{
	...
	for (ImGuiTableSettings *settings = g.SettingsTables.begin();
		...
		for (int column_n = 0; column_n < settings->ColumnsCount;
		     column_n++, column++) {
			// "Column 0  UserID=0x42AD2D21 Width=100 Visible=1 Order=0 Sort=0v"
			bool save_column = save_size || save_visible || save_order ||
			...
			if (column->ID != 0) {
				buf->appendf(" ID=0x%08X", column->ID);
			}
			buf->append("\n");
		}
...
void ImGui::TableGcCompactTransientBuffers(ImGuiTable *table)
{
	...
	table->IsSortSpecsDirty =
	    true;        // FIXME: In theory shouldn't have to leak into user
	                 // performing a sort on resume.
...
// Compact and remove unused or resize settings data (only TestEngine mark
// unused, but resizing table down can lead to compaction)
...
void ImGui::DebugNodeTable(ImGuiTable *table)
{
	...
	if (IsItemHovered())
		GetForegroundDrawList(table->OuterWindow)
		    ->AddRect(table->OuterRect.Min,
		              table->OuterRect.Max,
		              IM_COL32(255, 255, 0, 255));
	if (IsItemVisible() && table->HoveredColumnBody != -1)
		GetForegroundDrawList(table->OuterWindow)
		    ->AddRect(
		        GetItemRectMin(), GetItemRectMax(), IM_COL32(255, 255, 0, 255));
	if (!open)
		return;
	if (table->InstanceCurrent > 0)
	...
	BulletText("ResizedColumn: %d, HeldHeaderColumn: %d, ReorderColumn: %d",
	           table->LastResizedColumn,
	           table->LastHeldHeaderColumn,
	           table->ReorderColumn);
	...
	if (clear_settings)
		table->IsResetAllRequest =
		    true;        // Queue a call to TableResetSettings()
	TreePop();
}
                                   ...
                                   ImGuiTable         *table)
{
	if (settings->ID == 0)
		PushID(settings);
	const bool open    = TreeNode((void *) (intptr_t) settings->ID,
...
void ImGui::EndColumns()
{
	...
	if (!(flags & ImGuiOldColumnFlags_NoBorder) && !window->SkipItems) {
		...
		for (int n = 1; n < columns->Count; n++) {
			...
			if (!ItemAdd(
			        column_hit_rect, column_id, NULL, ImGuiItemFlags_NoNav))
				continue;
