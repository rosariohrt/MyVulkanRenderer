...
// this is why this is not the default). #define IMGUI_DISABLE_TIME_FUNCTIONS //
// Don't setup default platform_io.Platform_SessionDate value using time(),
// localtime_r(). #define IMGUI_DISABLE_DEFAULT_SHELL_FUNCTIONS             //
// Don't implement default platform_io.Platform_OpenInShellFn() handler (Win32:
// ShellExecute(), require shell32.lib/.a, Mac/Linux: use system("")). #define
...
// fonts (ProggyClean + ProggyForever). Remove ~9 KB + ~14 KB from output
// binary. AddFontDefaultXXX() functions will assert. #define
// IMGUI_DISABLE_DEFAULT_FONT_BITMAP                 // Disable default embedded
// bitmap font (ProggyClean). Remove ~9 KB from output binary.
// AddFontDefaultBitmap() will assert. #define IMGUI_DISABLE_DEFAULT_FONT_VECTOR
// // Disable default embedded vector font (ProggyForever), Remove ~14 KB from
// output binary. AddFontDefaultVector() will assert. #define IMGUI_DISABLE_SSE
...
// "imgui_te_imconfig.h", see Test Engine for details.
